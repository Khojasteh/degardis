...
- **The profile index is the only lookup the bundle keeps.** It survives because
  which profiles apply is decided by the situation, which only the running agent
  sees. A task never selects a profile, and removing the complete generated
  `profiles/` tree must leave `SKILL.md` and every task page byte-for-byte
  unchanged, so a profile missed, or matched wrongly, cannot change validity or
  failure.
...
- **The compiler owns no words but `wording.py`'s.** Every principle, knowledge
  unit, profile, and task body is the author's, read from the skill being
...
- **One set of findings, three renderings.** `validate`, the `inspect` line
  ...
  compilation, not on one output path.
...
- **The root's section order is fixed, and it carries nothing it can point at.**
  No knowledge, no profile catalog, no resource index; a section with nothing to
...
| An inspect dimension or one of its rows | `output.py`, `INSPECT_DIMENSIONS` in `validate.py`, `manual_topics/inspection.md` where the row shapes are stated, and the `inspect` section of `docs/cli.md` |
