...
Tests are standard-library `unittest`, discovered from `tests/`.
...
- **Every check code is a literal with an explanation, and every explanation has
  ...
  it and be explained by `explain_codes/<code>.yaml`. Both directions matter: a
  code assembled at runtime can no longer be checked against that table, and an
  entry no module can report describes a check the compiler never runs.
- **The explain table is data, and its filenames are its vocabulary.**
  ...
  is stated once in [degardis/explain_wording.yaml](degardis/explain_wording.yaml)
  and named by each of them. `explain.py` reads that data and states none of it,
  because the words belong to whoever repairs a source. Listing the codes reads
  no file's contents, which is what lets the CLI name them on every run without
  paying for prose it will not print.
...
- **A missing or invalid field reports a check naming the key.** The three
  ...
  or an advice item: it keeps the enclosing item's check, because there the item
  rather than the file is the unit an author repairs.
...
- **The manual has one source, and all of it is printable.**
  ...
  topic and does not belong there. `docs/manual.md` is generated from those
  topics and [tools/manual_template.md](tools/manual_template.md), which
  carries the document's own heading, lead, and notice and nothing a topic
  could carry instead. The generated copy is never edited, and CI fails a stale
  one.
...
- **The manual is one text for two readers**, `degardis manual TOPIC` for an
  ...
  a topic is written in the author's vocabulary and names no compiler internal.
...
- **The `explain` table** is for whoever repairs a source, agent or author: what
  ...
  care, which is why it is data a person edits rather than text a module builds.
  An example is source they copy, so it states its lines as lines.
...
| Change | Also update |
| --- | --- |
| A check | The literal code, its `explain_codes/<code>.yaml` file, and the manual topic stating the rule, where it changes what `validate` accepts |
| A construct's schema — a field, a phase, a step form, a selector key | The field set in `sources.py`, the check that rejects unknown keys, `lowering.py` where that field is enforced, `render.py` where it appears, and that construct's topic in `manual_topics/` |
...
| The DExpr grammar, a namespace, or a type | `dexpr.py`, the checks that report its codes, and `manual_topics/expressions.md` |
| What a scope binds, or where lowering puts it | `lowering.py`'s scope resolution and node placement, the checks in `validate.py` that hold every bound item to a node, `render.py`'s node shape, and `manual_topics/selectors.md` with the bound construct's own topic |
| A CLI option or command | The command's help and `Examples:` block in `cli.py`, `docs/cli.md`, and the command list in `README.md` |
| An inspect dimension or one of its rows | `output.py`, `INSPECT_DIMENSIONS` in `validate.py`, the `inspect` epilog legend, and the `inspect` section of `docs/cli.md` |
| The wording of a generated Markdown section | `wording.py` only; `render.py` holds the structure, not the words |
| A manual topic | Its file, `manual_topics/topics.yaml` where the name, summary, or cross-references change, and `docs/manual.md` regenerated |
| The generated bundle's layout | `package.py`, `render.py`'s page targets, the conformance cases that read it, and `docs/artifact-format.md` |
