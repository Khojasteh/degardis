...
What is deliberately *not* decided here is which profiles apply. That depends on
...
class TaskPlan:
    ...
    @property
    def closure(self) -> tuple[Knowledge, ...]:
        """Every unit compiled into this page, in its final grouped order."""
        return self.knowledge

    def of_kind(self, kind: str) -> tuple[Knowledge, ...]:
        """The units in one rendered subsection, preserving author order."""
        return tuple(unit for unit in self.knowledge if unit.kind == kind)

    @property
    def carries_material(self) -> bool:
        """Whether the page says anything beyond the goal it opens with.

        A task with no body, no knowledge, and no guide
        compiles to a page carrying its goal and nothing else, which the router
        entry that led there could have carried instead. That is a load an agent
        pays for one sentence, and it usually means the task was declared before
        the material that justifies it was written.
        """
        return bool(
...
@dataclass(frozen=True)
class Plan:
    """One skill's complete placement: its tasks, its principles, its profiles."""
    ...
    def categorized(self) -> bool:
        """Whether the profile index groups its rows.

        One category over every profile is not a grouping: it would add a
        heading that separates nothing. Two or more is, because then the
        category is what tells a reader which rows to read.
        """
        return len({item.category for item in self.profiles if item.category}) > 1
...
def _resolve_closure(
    task: Task, sources: SourceSet
) -> tuple[list[Knowledge], list[str], list[str], list[tuple[str, str]], list[tuple[str, ...]]]:
    ...
    return grouped, direct, required, unresolved, cycles
...
def plan_skill(
    ...
    for identifier in _route_order(sources, tasks):
        ...
        closure, direct, required, missing, found = _resolve_closure(task, sources)
    ...
    return Plan(
        ...
        profiles=tuple(item for _, item in sorted(sources.profiles.items())),
...
def measure(plan: Plan, sources: SourceSet) -> Quality:
    ...
    about the source, and the repair is a decision
    about expertise that belongs to whoever wrote it.
    """
