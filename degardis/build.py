...
from .inspection import promote_warnings, result_dict
...
from .validate import Inspection, compile_all
...
class SkillCompiler:
    ...
    def build(
        ...
        *,
        ...
        self.inspections = compile_all(self.skill_paths, watcher)
        ...
        paths: list[Path] = []
        for position, inspection in enumerate(self.inspections, start=1):
            counter = f" ({position}/{len(self.inspections)})" if len(self.inspections) > 1 else ""
            watcher.phase(f"Writing {inspection.skill.name}{counter}")
    ...
    def _commit(
        ...
        with TemporaryDirectory(prefix="degardis-build-") as directory:
            ...
            write_bundle(
                ...
                compiled.content.icon_addresses,
                compiled.content.icon_assets,
                staged_folder,
            )
            if as_zip:
                ...
                self.packager.create(
                    staged_folder,
                    staged,
                    executable_paths(skill.root, compiled.content.selected),
                )
            else:
...
def build_skills(
    ...
    *,
    ...
    return SkillCompiler(sources).build(
        output_dir, as_zip, fail_on_warning=fail_on_warning, progress=progress
    )
