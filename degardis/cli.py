"""The command-line surface: six commands, their help, and their exit status.
...
Examples:
  ...
  degardis manual layout yaml protocol
""",
...
the entrypoints a request is routed to — each named by the workflow it enters,
in the order an agent tries them, with the one taken when no other matched
marked `(default)` — and one count per selected content key. A `workflows` row
is `ID STATUS [from CALLER] N steps N nodes PATH BYTES`, where STATUS is entry,
...
failure. A `lowering` row is `KIND ID lowered|not-lowered NODES BYTES`: not-lowered
means a bound binding item reached no generated node, which is an error, since a
requirement no node states is a requirement no agent can act on. BYTES counts
rendered UTF-8 node blocks or inline constraint lines across all placements,
excluding shared module headings, blank separators, and other bindings' inline
constraints. It counts each emitted placement once, not runtime invocations. `attention`
...
they exclude SKILL.md, optional material, and resources. So no run costs
`path worst`: it is the dearest route, its bytes and its loads may come from
two different routes, and a module the path re-enters is charged again rather
than counted as still held. Read it against another build of the same source
to see whether a change helped; across two skills, or against a host's context
limit, it compares route shape and not what either run will read.
A `diagnostics` row is
...
`degardis manual` lists the authoring topics; `degardis manual TOPIC [TOPIC ...]`
prints the selected manual sections without needing a source path.
...
""",
    ...
    manual_command = subcommands.add_parser(
        ...
        description=(
            f"Read the authoring manual for source format {CURRENT_FORMAT_VERSION} shipped "
            "with this installation. With no topics, list available topics. "
            "Otherwise print each requested topic once, in request order, as "
            "Markdown with field tables and examples. Works offline, reads no "
            "skill source, needs no source path, and writes no files. Every "
            "known topic given is printed in one run; an unrecognized topic "
            "exits non-zero and lists every topic this version ships."
        ),
        epilog=f"""\
Topics:
{describe_topics()}

A cross-reference names the topic to read next; no topic links out of the CLI.
Unknown names are reported together, after every known topic has printed.

Examples:
  degardis manual
  degardis manual layout yaml content
  degardis manual protocol selectors values
""",
        formatter_class=HELP_FORMATTER,
    )
    ...
    if args.command == "inspect":
        ...
        with status_line(sys.stderr) as watcher:
            results = inspect_skills(
                skill_paths, include_body=args.body_text, progress=watcher
            )
