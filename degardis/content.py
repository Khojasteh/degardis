...
A manifest never defaults a content key to a directory: a key it leaves out says
the skill ships none of that content. So this module answers one question —
which files does this pattern list name? — and reports a pattern that names
nothing, because a misspelled path, a wrongly cased one, or a `!` that now
...
# first five select construct source; scripts and assets are unchanged files the
# bundle carries. Guides are separately loaded knowledge pages, not resources.
#
# Knowledge is one construct namespace. Its `kind` frontmatter field classifies
# how a unit renders on a task page; source directories do not create separate
# identities or schemas for concepts, facts, constraints, and guidance.
...
COPIED_CONTENT_KEYS: tuple[str, ...] = ("scripts", "assets")
CONTENT_KEYS: tuple[str, ...] = (*PARSED_CONTENT_KEYS, *COPIED_CONTENT_KEYS)
ALLOWED_CONTENT_KEYS = frozenset(CONTENT_KEYS)

# The inline reference kind each copied key answers, so `[[script:check.py]]`
# reaches a file selected by `content.scripts`. The renderer that offers these
# targets and the check that counts a reference as using a shipped file both
# read this and resolve targets through `resolve_copied`, because a disagreement
# between them would let a reference resolve on a page and still be reported as
# naming nothing.
COPIED_CONTENT_KINDS: dict[str, str] = {"scripts": "script", "assets": "asset"}


def resolve_copied(target: str, paths: Collection[str]) -> list[str]:
    """The selected files one script or asset target names, as source-relative paths.

    A target is a file's path, or any trailing run of its whole segments:
    `check.py` and `lint/check.py` both name `scripts/lint/check.py`. More than
    one match is the author's to settle by writing more of the path, so every
    match is returned rather than one picked. A path written out in full is the
    most specific address there is, so it names that file alone even where it
    is also the tail of a longer path.
    """
    if target in paths:
        return [target]
    return sorted(path for path in paths if path.endswith(f"/{target}"))


def shortest_address(path: str, paths: Collection[str]) -> str:
    """The shortest target that names one selected file and no other."""
    parts = path.split("/")
    for size in range(1, len(parts) + 1):
        candidate = "/".join(parts[-size:])
        if resolve_copied(candidate, paths) == [path]:
            return candidate
    return path

# The one content key a manifest must set. A task is the only thing a request is
# routed to, so a skill with no task has nothing for `SKILL.md` to route to.
REQUIRED_CONTENT_KEYS: tuple[tuple[str, str], ...] = (
    ("tasks", "content.missing-tasks"),
)
