...
class Binding:
    ...
    def render(self) -> str:
        ...
        return f"`{render_literal(self.literal)}`"
...
def parse_binding(data: Any) -> tuple[Binding | None, str]:
    ...
    if key == "literal":
        if isinstance(value, (dict, list)) or (
            ...
            return None, "literal takes a non-empty string, number, boolean, or null"
...
def render_literal(value: Any) -> str:
    """Spell one written literal the way a source wrote it.

    A binding and a produced value's default are both literals an author typed,
    and both reach the generated document, so both spell one here: two
    spellings would let a supplied `[]` and a defaulted `[]` read differently
    in the same file.
    """
    if value is None:
        return "null"
...
def constant_boolean(expression: Expression) -> bool | None:
    """Return a condition's compile-time truth where no runtime value decides it.

    The evaluator deliberately covers only Boolean composition. Comparisons and
    functions keep their runtime meaning even when their operands happen to be
    literals, so this check cannot drift into a second implementation of DExpr.
    """
    def evaluate(node: Any) -> bool | None:
        if isinstance(node, Grouping):
            return evaluate(node.inner)
        if isinstance(node, Literal) and isinstance(node.value, bool):
            return node.value
        if isinstance(node, Negation):
            found = evaluate(node.operand)
            return None if found is None else not found
        if isinstance(node, Conjunction):
            values = [evaluate(item) for item in node.operands]
...
class _Checker:
    ...
    def check_function(self, node: Function, established: set[str]) -> ValueType | None:
        ...
        if container.kind == "string":
            ...
            if container.item is not None and not _comparable(container.item, item.base):
                self.report(
                    TYPE_MISMATCH,
                    f"contains looks for a {item.render()} in a "
                    f"{container.render()}",
                )
            if container.item is not None:
                self.check_member(node.arguments[1], container.item)
        else:
            self.report(
                TYPE_MISMATCH,
    ...
    def check_member(self, node: Any, declared: ValueType) -> None:
        while isinstance(node, Grouping):
            node = node.inner
        if (
            declared.kind == "enum"
            and isinstance(node, Literal)
            and isinstance(node.value, str)
            and node.value not in declared.values
        ):
            self.report(
                "expr.unknown-member",
                f"{node.value!r} is not one of {', '.join(declared.values)}",
            )
    ...
    def expect_boolean(
        ...
        if found is not None and found.base.kind != "boolean":
            self.report(
                TYPE_MISMATCH, f"{operator} joins truths, not {_named(found)}"
            )
        return BOOLEAN
    ...
    def check_comparison(
        ...
        if node.operator in MEMBERSHIP:
            ...
            if container.kind == "list" and container.item is not None:
                self.check_member(node.left, container.item)
...
def check_expression(expression: Expression, environment: TypeEnvironment) -> list[Problem]:
    ...
    if found is not None and found.base.kind != "boolean":
        checker.report(
            TYPE_MISMATCH,
            f"a condition must be a truth, and this one is {_named(found)}",
        )
    return checker.problems


def check_default(value: Any, expected: ValueType) -> str:
    """Say why one written default cannot stand for its declared value, or nothing.

    A default is checked here rather than through `check_binding` because it is
    not a binding: it names no other value, so there is no environment to
    resolve it against, and it is read where the value is declared rather than
    where one is supplied.

    Two types take no default at all. An optional value is already absent where
    nothing produced it, so a default would give the same declaration two
    answers for the same path. A record has no literal form, so the only honest
    default for one is the record itself, which a step has to produce. An empty
    list is still a default for a list of records, because no field is written.
    """
    if expected.kind == "optional":
        return (
            "an optional value is already absent where no step produced it, so "
            "it takes no default"
        )
    if expected.kind == "record":
        return (
            f"{_named(expected)} has no literal form, so it takes no default; "
            "produce it on every path that reads it, or declare it optional"
        )
    if expected.kind == "list":
        if not isinstance(value, list):
            return f"{_written(value)} cannot stand for {_named(expected)}"
        if expected.item is None:
            return ""
        for index, item in enumerate(value):
            problem = check_default(item, expected.item)
            if problem:
                return f"item {index}: {problem}"
        return ""
    found = None if isinstance(value, (list, dict)) else _literal_type(value)
    if found is None or not _comparable(found, expected):
        return f"{_written(value)} cannot stand for {_named(expected)}"
    if expected.kind == "enum" and value not in expected.values:
        return f"{value!r} is not one of {', '.join(expected.values)}"
    return ""


def _written(value: Any) -> str:
    """How one written literal is named in a message about its type."""
    if value is None:
        return "null"
    if isinstance(value, list):
        return "a list"
    if isinstance(value, dict):
        return "a mapping"
    found = _literal_type(value)
    return _named(found) if found is not None else "null"


def _named(declared: ValueType) -> str:
    """One type as a message names it, with the article its spelling needs."""
    rendered = declared.render()
    return f"{'an' if rendered[:1] in 'aeiou' else 'a'} {rendered}"
...
def check_binding(
    ...
    if binding.kind == "literal":
        ...
        if binding.literal is None:
            if expected.kind != "optional":
                return [
                    Problem(
                        MISTYPED_BINDING, f"null cannot supply {_named(expected)}"
                    )
                ]
            return []
        if found is None or not _comparable(found, expected.base):
            rendered = "null" if found is None else _named(found)
            return [
                Problem(
                    MISTYPED_BINDING,
                    f"{rendered} cannot supply {_named(expected)}",
                )
            ]
    ...
    if (optional or found.optional) and expected.kind != "optional":
        return [
            Problem(
                MISTYPED_BINDING,
                f"{reference} may be absent, and {_named(expected)} must be "
                "present",
            )
        ]
    if not _comparable(found.base, expected.base):
        return [
            Problem(
                MISTYPED_BINDING,
                f"{reference} is {_named(found)}, and {_named(expected)} "
                "is expected",
            )
        ]
    return []
