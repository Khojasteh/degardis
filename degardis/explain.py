...
values for. No explanation is written in Python: this module reads that data,
...
def _versions() -> dict[str, str]:
    ...
    return {
        ...
        "facet_budget": _size_label(FACET_BUDGET_BYTES),
    }
...
def known_codes_message() -> str:
    """List every explainable code, one namespace to a line, for an error path.
    ...
    Neither is wrapped: the reader's window does that at its own width.
    """
    lines = [
        "A code is <namespace>.<check>, hyphenated, except where the check names "
        "a field of the source, which it spells exactly as the key does: "
        "interface.short_description-length.",
        "",
        "Known check codes:",
    ]
    for namespace, names in codes_by_namespace().items():
        lines.append("  " + ", ".join(f"{namespace}.{name}" for name in names))
    return "\n".join(lines)
