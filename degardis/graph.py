...
from .sources import Outcome, ProducedValue, SourceSet, Step, Workflow
...
class WorkflowGraph:
    ...
    # Every produced value that declares a default, keyed by the name it is read
    # under and ordered by the step that produces it, so what a document states
    # about them is the same on every host.
    defaults: dict[str, tuple[ValueType, Any]] = field(default_factory=dict)
...
def build_graph(
    ...
    for identifier in graph.order:
        ...
        if step.form != "use" or callee is None:
            continue
        for mapped in step.on:
            outcome = callee.outcome(mapped.id)
            record = sources.records.get(outcome.record) if outcome else None
            if not mapped.capture and record is not None and record.fields:
                diagnostics.warning(
                    f"{path}: steps.{identifier}.on.{mapped.id} discards the "
                    f"fields of record {record.id}; bind the outcome with as "
                    "to read them in the caller",
                    "workflow.unbound-outcome",
                    path,
                )
    return graph
...
def _declare_values(
    ...
    produced: dict[str, tuple[ProducedValue, str]] = {}

    def declare_default(value: ProducedValue, where: str) -> None:
        """Hold every producer of one value to the same default.

        A default belongs to the value, not to whichever step happens to write
        it, so two producers that disagree leave one name with two answers on
        the paths that reach neither. The repair is the same as for two
        conflicting types — make the declarations agree — so the check is too.
        """
        existing = produced.get(value.name)
        if existing is not None and existing[0].default != value.default:
            error(
                f"{where} declares result.{value.name} with "
                f"{_default_phrase(value.default)}, and {existing[1]} declares "
                f"it with {_default_phrase(existing[0].default)}",
                "workflow.conflicting-value",
            )
            return
        produced[value.name] = (value, where)
    ...
    for identifier in graph.order:
        ...
        if step.form == "action":
            for value in step.produces:
                where = f"steps.{identifier}.produces"
                declare(("result", value.name), value.type, where)
                declare_default(value, where)
    ...
    graph.types = types
    graph.defaults = {
        name: (types[("result", name)], value.default)
        for name, (value, _) in produced.items()
        if value.defaulted and ("result", name) in types
    }


def _default_phrase(default: Any) -> str:
    """How a message names one declared default, or the absence of one."""
    if default is None:
        return "no default"
    return f"the default `{render_literal(default)}`"
...
def _flow(workflow: Workflow, steps: dict[str, Step], graph: WorkflowGraph) -> None:
    ...
    for identifier in graph.order:
        ...
        if step.form == "action":
            produced[identifier] = frozenset(
                ("result", value.name) for value in step.produces
            )
        elif step.form == "decide":
    ...
    # A defaulted value is defined from the workflow's first node, which is the
    # whole of what lets a reader on a path that produced none read it at all.
    entry_values = frozenset(
        ("input", name) for name, _ in workflow.inputs
    ) | frozenset(("result", name) for name in graph.defaults)
...
def check_overwritten_values(
    ...
    diagnostics: Diagnostics,
    *,
    before_reads: dict[str, set[str]] | None = None,
    after_reads: dict[str, set[str]] | None = None,
) -> None:
    """Report a result definition replaced before any declared read consumes it.

    Pending definitions are unioned at joins: if one path reaches a second
    definition without consuming the first, that concrete path loses a value.
    Mutually exclusive definitions that meet only after both were written do
    not encounter each other and remain valid. Call captures are edge writes,
    because only the outcome that carries a record defines the captured value.

    Lowering supplies reads before and after an action's production for reached
    workflows. An unreached workflow has no lowered binding checks, so its own
    source reads are the complete set available to this check.
    """
    pending: dict[str, dict[str, frozenset[str]]] = {}
    reported: set[tuple[str, str]] = set()

    def read_names(step: Step) -> set[str]:
        found: set[str] = set()
        ...
        for case in step.cases:
            if case.when is not None:
                texts.extend(item.render() for item in references(case.when))
        for identifier in step.heuristics:
            heuristic = sources.heuristics.get(identifier)
            if heuristic is None:
                continue
            for advice in heuristic.advice:
                if advice.when is not None:
                    texts.extend(item.render() for item in references(advice.when))
        for text in texts:
            try:
    ...
    for identifier in graph.order:
        ...
        for predecessor in graph.predecessors.get(identifier, ()):
            for name, producers in pending.get(predecessor, {}).items():
                incoming.setdefault(name, set()).update(producers)
            previous_step = steps[predecessor]
            if previous_step.form != "use":
                continue
            for mapped in previous_step.on:
                if mapped.next != identifier or not mapped.capture:
                    continue
                name = mapped.capture
                previous = pending.get(predecessor, {}).get(name, frozenset())
                location = f"{predecessor}.on.{mapped.id}"
                if previous and (location, name) not in reported:
                    diagnostics.warning(
                        f"{workflow.path}: steps.{location}.as captures "
                        f"result.{name} before the definition from "
                        f"{', '.join(sorted(previous))} is read, so the "
                        "earlier value is lost",
                        "value.overwritten",
                        workflow.path,
                    )
                    reported.add((location, name))
                incoming.setdefault(name, set()).add(location)
        reads_before = (
            before_reads.get(identifier, set())
            if before_reads is not None
            else read_names(steps[identifier])
        )
        for name in reads_before:
            incoming.pop(name, None)
        produced = {
            name for namespace, name in graph.produced.get(identifier, ())
            if namespace == "result"
        }
        for name in sorted(produced):
            previous = incoming.get(name, set())
            if previous and (identifier, name) not in reported:
                diagnostics.warning(
                    f"{workflow.path}: steps.{identifier} produces result.{name} "
                    f"before the definition from {', '.join(sorted(previous))} "
                    "is read, so the earlier value is lost",
                    "value.overwritten",
                    workflow.path,
                )
        ...
        if after_reads is not None:
            for name in after_reads.get(identifier, set()):
                incoming.pop(name, None)
        pending[identifier] = {
            name: frozenset(producers) for name, producers in incoming.items()
        }
...
def call_order(
    roots: Sequence[str], sources: SourceSet, diagnostics: Diagnostics
) -> tuple[tuple[str, ...], dict[str, tuple[str, str]]]:
    """The workflows a run can reach, in first-call order, and who calls each.

    A generated document renders each entrypoint target and then each supporting
    workflow in the order a run first reaches it, so an agent reading forward
    meets a callee after the call that named it. Roots come first, in the order
    the manifest declares them, because that is the order the routing table
    offers them in. A cycle is rejected: a call frame that could re-enter itself
    has no bound the compiler can render.

    A root is reached by activation rather than by a call, so it never enters
    `callers` — including a root some other root also calls, whose entry an
    agent can reach two ways and whose report row would otherwise claim the
    call was the only one.
    """
    ordered: list[str] = []
    callers: dict[str, tuple[str, str]] = {}
    known = [root for root in roots if root in sources.workflows]
    if not known:
        return (), {}
    pending = list(dict.fromkeys(known))
    entered = set(pending)
    while pending:
        ...
        for step in workflow.steps:
            if step.form != "use" or step.call not in sources.workflows:
                continue
            if step.call not in callers and step.call not in entered:
                callers[step.call] = (current, step.id)
            if step.call not in ordered and step.call not in pending:
                pending.append(step.call)
    _reject_call_cycles(tuple(sources.workflows), sources, diagnostics)
...
def _reject_call_cycles(
    ...
    """Walk from every root, so a cycle no first root reaches is still found.

    One shared `state` across the roots keeps a workflow both of them call from
    being walked twice, and reports each cycle once rather than once per root
    that can reach it.
    """
    state: dict[str, int] = {}
    ...
    for root in roots:
        if root in sources.workflows and state.get(root) is None:
            visit(root, [root])
