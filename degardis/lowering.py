...
@dataclass(frozen=True)
class BoundItem:
    """One bound provision or rule, rendered as a command beside the others.

    Two positions render an item this way. A `during` item lands on the node
    whose command it bounds, because it has nowhere else to go. A `before`,
    `after`, or `before-return` item lands on the check node its construct
    generated, and lands there beside its siblings: several provisions of one
    policy reaching one boundary are one node listing its items rather than one
    node each, so the shape they render in is the shape a `during` item already
    had.

    The construct and its local id are carried rather than a rendered sentence,
    because the inspect report indexes an item by the provision it came from,
    and the renderer is the only part that decides how to spell one.

    The activation condition and the verification travel with the command
    because adjacency is the whole of what pairs them. A node carries several
    items, and an item that arrived with only its sentence would leave the
    author's proof out of the document and out of every check that reads one.
    """
...
class Node:
    ...
    state_update: str = ""
    prohibits: bool = False
    invariants: tuple[BoundItem, ...] = ()
    # The items of a generated check node that carries more than one.
    checks: tuple[BoundItem, ...] = ()
    consider: tuple[str, ...] = ()
...
class Lowerer:
    ...
    def __init__(
        ...
        self.roots = frozenset(skill.roots)
    ...
    def _lower_step(
        ...
        for frame in opening:
            prefix.append(self._initialize_frame_node(workflow, step, frame))
    ...
    def _opens_at(self, frame: Frame, workflow: Workflow, step: Step) -> bool:
        ...
        return workflow.id in self.roots and step.id == workflow.entry
    ...
    def _closes_at(self, frame: Frame, workflow: Workflow, step: Step) -> bool:
        ...
        if frame.scope == "workflow":
            return True
        return workflow.id in self.roots

    def _initialize_frame_node(
        self, workflow: Workflow, step: Step, frame: Frame
    ) -> Node:
        """Make a frame's source-declared starting state executable.

        Keeping initialization in its own node ensures a frame with no enter
        hook still tells the agent which lifecycle state and data values exist.
        """
        ...
        for declared in protocol.data:
            if declared.defaulted:
                updates.append(
                    wording.STATE_SET.format(
                        name=declared.name,
                        binding=f"`{render_literal(declared.default)}`",
                    )
                )
            elif declared.type.kind == "optional":
                updates.append(wording.STATE_CLEAR.format(name=declared.name))
        return Node(
            label=node_label(
                workflow.id, step.id, f"{frame.label}-initialize"
            ),
            kind="initialize",
            command=wording.INITIALIZE_FRAME.format(
                frame=frame.name, state=protocol.initial
            ),
            workflow=workflow.id,
            step=step.id,
            source=f"protocol `{protocol.id}`, {frame.name}, opening state",
            state_update="; ".join(updates),
            transitions=(Transition("On completion"),),
    ...
    def _anchors(
        ...
        self._bind_invariants(workflow, graph, step, node, scopes, facts)
    ...
    def _source_node(
        ...
        if step.form == "action":
            ...
            node.produces = tuple(
                self._render_value(value.name, value.type) for value in step.produces
            )
            node.transitions = (
    ...
    def _phase_checks(
        ...
        for scope, identifier in scopes.policies:
            ...
            if policy is None:
                continue
            # Every provision one policy places at one boundary goes on one
            # node, in the order the policy declares them.
            selected = [
                (provision, provision.id)
                for provision in policy.provisions
                if provision.phase == phase
                and provision.selector.matches(anchor.facts)
            ]
            if not selected:
                continue
            for provision, _ in selected:
                self.result.matched_provisions.add((policy.id, provision.id))
                self.result.lowered_provisions.add((policy.id, provision.id))
            nodes.append(
                self._check_node(
                    workflow, graph, step, anchor, phase, edge,
                    "policy", policy.id, selected, scope,
                )
            )
        for scope, identifier in scopes.rules:
            ...
            # A rule is one provision, so its identity is the rule's own id and
            # it never has a sibling to share a node with.
            nodes.append(
                self._check_node(
                    workflow, graph, step, anchor, phase, edge,
                    "rule", rule.id, [(provision, "")], scope,
                )
            )
    ...
    def _check_node(
        ...
        kind: str,
        construct: str,
        items: list[tuple[Provision, str]],
        scope: str,
    ) -> Node:
        """One node for everything this construct places at this boundary.

        A lone item states its own sentence as the heading, because no sentence
        the compiler could write over it would say more, and the label keeps the
        item's local id. Several items share a generated heading and render
        beneath it, and the label drops the local id: the node is the
        construct's placement at this boundary rather than any one provision's,
        which is also what keeps it derived from source identity alone.

        Grouping stops at the construct. One node carries one construct's
        items, so `inspect` can still say which construct is speaking, the
        bytes it costs are still charged to it, and two constructs that happen
        to meet here are not made to look like one requirement.

        A failure still names the item rather than the node: the blocked edge
        asks for the command or verification that failed, which is a sentence
        on this node whether the node carries one item or ten.
        """
        after = phase == "after"
        environment = graph.environment(self.records, step.id, after=after)
        for provision, _ in items:
            self._check_activation(workflow, provision, environment, construct, kind)
        grouped = len(items) > 1
        local = "" if grouped else items[0][1]
        ...
        reads: set[str] = set()
        for provision, _ in items:
            reads.update(self._provision_reads(provision))
        bound = tuple(
            BoundItem(
            ...
            for provision, item_local in items
        )
        return Node(
            label=node_label(workflow.id, step.id, suffix),
            kind="check",
            command=wording.CHECK_GROUP[phase] if grouped else bound[0].command,
            workflow=workflow.id,
            step=step.id,
            source=f"{origin}, bound at {scope} scope, {where}",
            available=tuple(sorted(reads)),
            subjects=anchor.facts.subjects,
            effects=anchor.facts.effects,
            verify="" if grouped else bound[0].verify,
            activation="" if grouped else bound[0].activation,
            prohibits=False if grouped else bound[0].prohibits,
            checks=bound if grouped else (),
            transitions=(
            ...
            if heuristic is not None:
                for advice in heuristic.advice:
                    if advice.when is not None:
                        found.update(ref.render() for ref in references(advice.when))
        return tuple(sorted(found))
    ...
    def _bind_invariants(
        ...
        """Bind the `during` provisions and rules this node's command is under.
        ...
        This is also where the item's activation condition and verification are
        rendered and checked, because the node it lands on is the only position
        it has. `_check_node` does the same for every other phase against its
        own generated node; here the host node is that node, so the values the
        provision reads join the ones the step declared. The environment is the
        one entering the step rather than the one leaving it: a `during` item is
        proved as the command is performed, so it sees what `before` sees.

        Nothing is computed until an item actually lowers, so a node no
        `during` item selects pays neither the environment nor the checks.
        """
        takes_invariant = facts.form in ("action", "call", "return", "pattern")
        found: list[BoundItem] = []
        reads: set[str] = set()
        environment: TypeEnvironment | None = None

        def bound(provision: Provision, kind: str, construct: str, local: str) -> BoundItem:
        ...
        for _, identifier in scopes.policies:
            ...
            for provision in policy.provisions:
                ...
                found.append(bound(provision, "policy", policy.id, provision.id))
        for _, identifier in scopes.rules:
            rule = self.sources.rules.get(identifier)
            if rule is None:
                continue
            ...
            if not takes_invariant:
                ...
                continue
            self.result.lowered_rules.add(rule.id)
            found.append(bound(provision, "rule", rule.id, ""))
        node.invariants = tuple(found)
        if reads:
            node.available = tuple(sorted(set(node.available) | reads))
...
def _state_update_line(hook: Hook) -> str:
    parts = [
        wording.STATE_SET.format(name=name, binding=binding.render())
        for name, binding in hook.updates
    ]
    parts.extend(wording.STATE_CLEAR.format(name=name) for name in hook.clears)
    if hook.to:
...
def _resolve_transitions(result: LoweredSkill) -> None:
    ...
    for lowered in result.workflows:
        ...
        destinations = {node.label: node for node in lowered.nodes}
        for node in lowered.nodes:
            ...
            for transition in node.transitions:
                ...
                resolved.append(
                    Transition(
                        transition.label,
                        target,
                        destinations[target].command if target in destinations else "",
                        blocked=transition.blocked,
                        prohibits=(
                            target in destinations
                            and destinations[target].kind == "check"
                            and destinations[target].prohibits
                        ),
                    )
                )
