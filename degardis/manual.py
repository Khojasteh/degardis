...
# libyaml's loader where PyYAML was built with it, and its own otherwise. The
# catalog is data this package ships beside this file, not a source anyone
# authored, so the strict subset a skill source is held to has nothing to decide
# here and neither reading can differ from the other. What differs is the cost:
# the catalog is read whenever the CLI builds its help, which every command does,
# and the pure-Python loader spends an order of magnitude more of that run on a
# file whose contents are fixed at release.
CATALOG_LOADER = getattr(yaml, "CSafeLoader", yaml.SafeLoader)
...
def _resolve(text: str) -> str:
    """Substitute the format version at read time.

    A manual file states the placeholder, so the version the compiler accepts is
    the only one any reader can be shown.
    """
...
def _load_catalog() -> tuple[Topic, ...]:
    ...
    is generated from.
    """
    catalog = yaml.load(_resource(CATALOG), Loader=CATALOG_LOADER)
    return tuple(
