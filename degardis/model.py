...
# The one spelling of a name in this format: a skill name, a construct id, and
# every reference to either. One rule rather than one per position, so an author
# who has learned how to name a skill already knows how to name a file.
NAME_PATTERN = re.compile(r"^[a-z0-9]+(?:-[a-z0-9]+)*$")


def is_id(value: object) -> bool:
    """Whether one list item names something, once surrounding space is ignored."""
    return isinstance(value, str) and NAME_PATTERN.fullmatch(value.strip()) is not None
...
class Skill:
    ...
    and the interface metadata a target renders. The task, facet, and knowledge
    files the manifest selects are resolved separately, because a manifest whose
    content selection is wrong is still a manifest whose identity a report can
    name.

    `tasks` is the order the author routes in, not the files themselves: the
    ids as declared, before anything has confirmed that a file answers each one.
    """
    ...
    def purpose(self) -> str:
        """What this skill is for, in the words its own agent reads first.

        `description` is written for the host that selects among many skills;
        this is written for the agent that has already been selected and is
        about to choose a task. They are different sentences to different
        readers, so a manifest states both, and a manifest that states only the
        first has the description stand in rather than leaving the generated
        root with no orientation at all.
        """
        value = self.manifest.get("purpose")
