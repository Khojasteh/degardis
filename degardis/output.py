...
def _entrypoint_summary(result: dict[str, Any]) -> str:
    """The routing table as one line, in the order a run tries the routes.

    A reader scanning a list of skills wants to know what this one can be asked
    for, so each route is named by its target and the default is marked. The
    order is the manifest's, because that is the order an agent matches in.
    """
    routes = result.get("entrypoints") or []
    if not routes:
        return "Not specified"
    return ", ".join(
        f"{route['target']} (default)" if route.get("default") else str(route["target"])
        for route in routes
    )
...
def write_skill_list(stream: TextIO, results: list[dict[str, Any]]) -> None:
    ...
    for result in results:
        ...
        _write_field(stream, "Entrypoints", _entrypoint_summary(result))
        _write_field(
            stream,
            "Constructs",
            ", ".join(
...
def _write_inspect_skill(
    ...
    section(
        f"entry {_entrypoint_summary(result)}",
        "count "
        + ", ".join(
    ...
    if "profiles" in dimensions:
        ...
        for row in result["profiles"]:
            ...
            section(
                ...
                f"points={row['points']} | references={row['references']}"
            )
