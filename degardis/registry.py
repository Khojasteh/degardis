...
# What one entrypoint may declare. There is no field marking the route taken
# when nothing matched: a route with no `when` is that route, and it has to be
# the last one written, because anything after a route with no condition could
# never be reached.
ENTRYPOINT_FIELDS = frozenset({"when", "target", "with"})
...
def check_manifest(skill: Skill, diagnostics: Diagnostics) -> dict:
    ...
    _check_entrypoints(skill, diagnostics)
...
def _check_entrypoints(skill: Skill, diagnostics: Diagnostics) -> None:
    """Hold the routing table to what an agent must be able to act on.

    An entrypoint is chosen before any workflow runs, from the request alone, so
    each of its fields fails in its own way and reports its own check: a route
    with no target names nowhere to go, and a route the agent cannot tell the
    request against can never be taken. Those are separate repairs, which is why
    they are separate codes rather than one invalid-entrypoint.

    That per-field spelling follows `interface` rather than the per-item codes a
    step or a provision carries. Those sit inside a construct file, where the
    item is the unit an author repairs; an entrypoint sits in the manifest,
    where `interface` already establishes that a nested mapping names its own
    fields.

    Position, not a flag, says which route is the unconditional one: a route
    with no `when` is taken whatever the request says, so nothing after it could
    ever be reached. That is the rule a `branch` already holds its `otherwise`
    to, and it states the fact once, where a flag beside the absent `when` would
    state it twice and could disagree with itself.
    """
    path = skill.root / "skill.yaml"
    ...
    if "entrypoints" not in skill.manifest:
        return
    if not isinstance(declared, dict) or not declared:
        error(
            "entrypoints must be a non-empty mapping of route names to routes",
            "manifest.invalid-entrypoints",
        )
        return

    names = list(declared)
    for position, identifier in enumerate(names):
        body = declared[identifier]
        where = f"entrypoints.{identifier}"
        if not isinstance(identifier, str) or not NAME_PATTERN.fullmatch(identifier):
            error(
                f"entrypoints names {identifier} as a route; a route name is "
        ...
        if not isinstance(body, dict):
            error(
                f"{where} must be a mapping naming a target and when it applies",
                "entrypoint.invalid-target",
            )
            continue
        unknown = sorted(set(body) - ENTRYPOINT_FIELDS)
        if unknown:
            error(
                f"{where}: unrecognized entrypoint fields: {', '.join(unknown)}",
                "entrypoint.unknown-field",
            )

        target = body.get("target")
        if "target" not in body:
            error(f"{where}.target is required", "entrypoint.missing-target")
        elif (
            ...
            error(
                f"{where}.target must name a workflow file stem, in lowercase "
                "letters, digits, and single hyphens",
                "entrypoint.invalid-target",
            )
        ...
        if "when" not in body and not last:
            following = ", ".join(names[position + 1 :])
            error(
                f"{where} states no when, so it is taken whatever the request "
                f"says and {following} can never be reached; state when this "
                "route applies, or move it last to make it the route taken when "
                "no other matched",
                "entrypoint.missing-when",
            )
        elif "when" in body and (not isinstance(when, str) or not when.strip()):
            error(
                f"{where}.when must be a non-empty sentence stating when this "
                "route applies",
                "entrypoint.invalid-when",
            )
