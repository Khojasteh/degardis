...
from .model import CURRENT_FORMAT_VERSION, DegardisError, filename_title
from .registry import NAME_PATTERN
...
tasks:
# Every task, in the order the agent reads them when routing a request. Put the
# work most requests ask for first.
- {STARTER_TASK}
content:
  tasks:
  - tasks/*.md
...
title: {title}
recognize:
- the requester asks for the work this task performs
- name a second way the same request is phrased
goal: State what is true once this task is done.
---

Write the guidance that belongs to this task and to no other here. Anything a
second task would also need belongs in a knowledge unit instead, so that both
tasks compile the same text and neither copy can drift.
...
Guidance that holds across several of this skill's tasks goes in `principles/`,
one file per principle. Name the ones this skill uses in `skill.yaml`:
