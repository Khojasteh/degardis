...
from .model import NAME_PATTERN, Diagnostics, is_id
...
def read_task(source: MarkdownSource, diagnostics: Diagnostics) -> Task | None:
    ...
    guides = reader.names("guides", "task.invalid-guides")
    if not reader.ok:
        return None
    return Task(
        id=source.id,
...
def read_knowledge(
    ...
    if not source.body.strip():
        reader.error(
            ...
            "page as a heading with nothing under it; selected knowledge must "
            "carry content below the frontmatter",
            "knowledge.empty",
        )
    if not reader.ok:
        return None
...
def read_facet(source: MarkdownSource, diagnostics: Diagnostics) -> Facet | None:
    ...
    guides = reader.names("guides", "facet.invalid-guides")
    if not source.body.strip():
        reader.error(
...
_READERS: dict[str, Callable[[MarkdownSource, Diagnostics], Any | None]] = {
    "tasks": read_task,
    "principles": read_principle,
...
def read_construct(
    ...
    An unrecognized key raises, as `SourceSet.kind` does: reading a file under
    some other key's schema would file it under a namespace no reference names.
    """
    return _READERS[key](source, diagnostics)
