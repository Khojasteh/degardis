...
INSPECT_DIMENSIONS: dict[str, str] = {
    "skill": (
        "name, version, title, root, description length, the entrypoints a "
        "request is routed to, and selected construct counts"
    ),
    ...
    "lowering": (
        ...
        "into, its rendered UTF-8 bytes, or that it matched nothing"
    ),
    ...
    "attention": (
        "root control-plane bytes, execution-module bytes/count/maximum, "
        "worst-path execution bytes and loads, supplementary "
...
def _check_resource_uses(content: SkillContent, diagnostics: Diagnostics) -> None:
    ...
    for workflow in content.sources.workflows.values():
        for step in workflow.steps:
            if step.resource is None:
                continue
            consumed.add(step.resource.path)
    ...
    for key in ("assets", "scripts"):
        for relative in sorted(set(content.relative(key)) - consumed):
            path = content.skill.root / relative
            diagnostics.warning(
                f"{path}: content.{key} selects {relative}, but no resource "
...
def _check_conflicting_state_updates(
    content: SkillContent, diagnostics: Diagnostics
) -> None:
    """Reject one hook assigning and clearing the same data field."""
    for protocol in content.sources.protocols.values():
        for hook in protocol.hooks:
            overlap = sorted({name for name, _ in hook.updates} & set(hook.clears))
            if not overlap:
                continue
            fields = ", ".join(f"state.{name}" for name in overlap)
            diagnostics.error(
                f"{protocol.path}: hook {hook.id} both sets and clears {fields} "
                "at one boundary",
                "protocol.conflicting-state-update",
                protocol.path,
            )
...
def _check_constant_conditions(
    ...
    """Warn where a source condition makes its own execution statically dead."""
    sources = content.sources

    def report_constant(
        path: Path, location: str, expression: Any, effect: str
    ) -> None:
        diagnostics.warning(
            f"{path}: {location} is always "
            f"{'true' if constant_boolean(expression) else 'false'}, so {effect}",
            "expr.constant-condition",
            path,
        )
        ...
        for step in workflow.steps:
            ...
            for index, case in enumerate(step.cases):
                if case.when is None:
                    continue
                value = constant_boolean(case.when)
                if value is False:
                    report_constant(
                        workflow.path,
                        f"steps.{step.id}.branch[{index}].when",
                        case.when,
                        "that case can never be selected",
                    )
                elif value is True and index + 1 < len(step.cases):
                    report_constant(
                        workflow.path,
                        f"steps.{step.id}.branch[{index}].when",
                        case.when,
                        "every following case is unreachable",
                    )

    provisions = [
        (policy.path, f"provisions.{item.id}", item)
        for policy in sources.policies.values()
        for item in policy.provisions
    ]
    provisions.extend(
        (rule.path, "rule", rule.provision) for rule in sources.rules.values()
    )
    for path, location, provision in provisions:
        if provision.when is not None and constant_boolean(provision.when) is False:
            report_constant(
                path,
                f"{location}.when",
                provision.when,
                "the command is never active",
            )
        if provision.unless is not None and constant_boolean(provision.unless) is True:
            report_constant(
                path,
                f"{location}.unless",
                provision.unless,
    ...
    for protocol in sources.protocols.values():
        for hook in protocol.hooks:
            if hook.when is not None and constant_boolean(hook.when) is False:
                report_constant(
                    protocol.path,
                    f"hooks.{hook.id}.when",
                    hook.when,
                    "the hook can never run",
                )
    for heuristic in sources.heuristics.values():
        for advice in heuristic.advice:
            if advice.when is not None and constant_boolean(advice.when) is False:
                report_constant(
                    heuristic.path,
                    f"advice.{advice.id}.when",
                    advice.when,
                    "the advice can never apply",
                )


def _check_unused_construct_values(
    content: SkillContent, diagnostics: Diagnostics
) -> None:
    """Warn about declared pattern and protocol values no source operation uses."""
    for pattern in content.sources.patterns.values():
        read: set[str] = set()
        for item in pattern.procedure:
            for text in item.uses:
                try:
                    ...
                    continue
                if reference.namespace == "input" and reference.path:
                    read.add(str(reference.path[0]))
        for name, _ in pattern.inputs:
            if name not in read:
                diagnostics.warning(
                    f"{pattern.path}: pattern {pattern.id} input.{name} is "
                    "declared and no procedure item reads it",
                    "value.unread",
                    pattern.path,
                )
    ...
    for protocol in content.sources.protocols.values():
        used: set[str] = set()
        for hook in protocol.hooks:
            expressions = [hook.when]
            if hook.verify is not None:
                expressions.append(hook.verify.expression)
            for expression in expressions:
                ...
                used.update(
                    ...
                    if reference.namespace == "state" and reference.path
                )
            for _, binding in hook.updates:
                reference = binding.reference
                if (
                    reference is not None
                    and reference.namespace == "state"
                    and reference.path
                ):
                    used.add(str(reference.path[0]))
            used.update(name for name, _ in hook.updates)
            used.update(hook.clears)
        for declared in protocol.data:
            if declared.name not in used:
                diagnostics.warning(
                    f"{protocol.path}: protocol {protocol.id} data.{declared.name} "
                    "is declared and no hook reads, sets, or clears it",
                    "value.unread",
                    protocol.path,
                )
...
def compile_skill(
    ...
    _check_entrypoint_bindings(content, diagnostics)
    _check_resource_uses(content, diagnostics)
    _check_conflicting_state_updates(content, diagnostics)
    _check_constant_conditions(content, diagnostics)
    _check_unused_construct_values(content, diagnostics)
    # One finding per route that names nowhere, not one for the manifest: two
    # bad targets are two repairs, and a message naming only the file would make
    # them one record and drop the second.
    missing = [
        item
        for item in skill.entrypoints
        if item.target and item.target not in sources.workflows
    ]
    for item in missing:
        diagnostics.error(
            f"{skill.root / 'skill.yaml'}: entrypoints.{item.id} targets "
            f"{item.target}, which content.workflows does not select",
    ...
    order, callers = call_order(skill.roots, sources, diagnostics)
    _warn_unreached_workflows(content, order, diagnostics)
    for identifier, workflow in sources.workflows.items():
    ...
    if any(not graph.usable for graph in result.graphs.values()):
        return result
    watcher.phase(f"Lowering {skill.name}")
    lowerer = Lowerer(skill, sources, result.graphs, order, callers, diagnostics)
    result.lowered = lowerer.lower()
    _check_overwritten_results(result.lowered, diagnostics)
    for identifier, workflow in sources.workflows.items():
        if identifier in order:
            continue
        check_overwritten_values(
            workflow,
            {step.id: step for step in workflow.steps},
            sources,
            result.graphs[identifier],
            diagnostics,
        )
    _check_unread_values(result.lowered, diagnostics)
    _check_ordinary_identifiers(result.lowered, diagnostics)
    ...
    _check_protocol_data_flow(result.lowered, diagnostics)
    result.rendered = render_skill(result.lowered, diagnostics, progress=watcher)
    _check_attention_budgets(result, diagnostics)
    _check_outputs(result, diagnostics)
    return result


def _check_overwritten_results(
    lowered: LoweredSkill, diagnostics: Diagnostics
) -> None:
    """Use lowered reads so a bound check can consume a pending result."""
    for item in lowered.workflows:
        before: dict[str, set[str]] = {}
        after: dict[str, set[str]] = {}
        for step in item.workflow.steps:
            nodes = [node for node in item.nodes if node.step == step.id]
            boundary = len(nodes)
            if step.form == "action":
                boundary = next(
                    (
                        ...
                        if node.origin == "action"
                    ),
                    len(nodes),
                )
            for destination, selected in (
                (before, nodes[:boundary]),
                (after, nodes[boundary:]),
            ):
                for node in selected:
                    for text in node.available:
                        try:
                            reference = parse_reference(text)
                        except ExpressionError:
                            continue
                        if reference.namespace == "result" and reference.path:
                            destination.setdefault(step.id, set()).add(
                                str(reference.path[0])
                            )
        check_overwritten_values(
            item.workflow,
            {step.id: step for step in item.workflow.steps},
            lowered.sources,
            item.graph,
            diagnostics,
            before_reads=before,
...
def _check_unread_values(lowered: LoweredSkill, diagnostics: Diagnostics) -> None:
    """Count declared reads after lowering so bound checks can consume results."""
    for item in lowered.workflows:
        ...
        for node in item.nodes:
            for text in node.available:
                try:
                    ...
                    continue
                if reference.path:
                    read.add((reference.namespace, str(reference.path[0])))
        for step in item.workflow.steps:
            if step.id not in item.graph.reachable:
                continue
            produced = [value.name for value in step.produces]
            produced.extend(mapped.capture for mapped in step.on if mapped.capture)
            for name in produced:
                if ("result", name) not in read:
                    diagnostics.warning(
                        f"{item.workflow.path}: steps.{step.id} produces "
                        f"result.{name}, which no reachable declared read consumes",
                        "value.unread",
                        item.workflow.path,
                    )
        for name, _ in item.workflow.inputs:
            if ("input", name) not in read:
                diagnostics.warning(
                    f"{item.workflow.path}: input.{name} is declared and no "
                    "reachable declared read consumes it",
                    "value.unread",
                    item.workflow.path,
                )
    for workflow in lowered.sources.workflows.values():
        if workflow.id in lowered.reached:
            continue
        read: set[str] = set()
        for step in workflow.steps:
            texts = list(step.uses)
            texts.extend(
                binding.reference.render()
            ...
            for case in step.cases:
                if case.when is not None:
                    texts.extend(
                        reference.render() for reference in references(case.when)
                    )
            for text in texts:
                try:
                    reference = parse_reference(text)
                ...
                if reference.namespace == "input" and reference.path:
                    read.add(str(reference.path[0]))
        for name, _ in workflow.inputs:
            if name not in read:
                diagnostics.warning(
                    f"{workflow.path}: input.{name} is declared and no "
                    "declared source read consumes it",
                    "value.unread",
                    workflow.path,
                )


def _check_ordinary_identifiers(lowered: LoweredSkill, diagnostics: Diagnostics) -> None:
    """Keep prose separate from schema ids and expressions when warning on names."""
    identifiers = {
        identifier
        for kind in CONSTRUCT_KINDS if kind != "profiles"
        for identifier in lowered.sources.kind(kind)
    }
    prose_fields = {
        "title", "summary", "description", "purpose", "command", "confirm",
        "question", "prefer", "because", "caution", "points",
    }
    reachable_steps = {item.workflow.id: item.graph.reachable for item in lowered.workflows}
    code_span = re.compile(r"(?<!`)(`+)(?!`)[\s\S]*?(?<!`)\1(?!`)")

    def check(text: str, path: Path, location: str) -> None:
        tokens = set(re.findall(r"(?<![\w/-])[a-z0-9]+(?:-[a-z0-9]+)*(?![\w/-])", code_span.sub(" ", text)))
        for identifier in sorted(tokens & identifiers):
            diagnostics.warning(
                f"{path}: {location} uses construct id {identifier} as ordinary "
                "prose; use backticks for an identifier or reword an ordinary word",
                "source.ordinary-identifier",
                path,
            )

    def walk(value: Any, path: Path, location: str) -> None:
        if not is_dataclass(value):
            return
        for member in fields(value):
            child = getattr(value, member.name)
            where = f"{location}.{member.name}"
            if member.name in prose_fields:
                if isinstance(child, str):
                    check(child, path, where)
                elif isinstance(child, tuple):
                    for index, text in enumerate(child):
                        check(text, path, f"{where}[{index}]")
            elif is_dataclass(child):
                walk(child, path, where)
            elif isinstance(child, tuple):
                for index, item in enumerate(child):
                    if member.name == "steps" and item.id not in reachable_steps.get(value.id, ()):
                        continue
                    if member.name == "provisions" and (value.id, item.id) not in lowered.lowered_provisions:
                        continue
                    if member.name == "hooks" and (value.id, item.id) not in lowered.lowered_hooks:
                        continue
                    walk(item, path, f"{where}.{getattr(item, 'id', index)}")

    check(lowered.skill.description, lowered.skill.root / "skill.yaml", "description")
    for kind, reached in lowered.reached_constructs().items():
        for identifier in sorted(reached):
            source = lowered.sources.kind(kind).get(identifier)
            if source is not None:
                walk(source, source.path, f"{kind}.{identifier}")
...
def _declared_types(construct: Any) -> list[tuple[str, Any]]:
    ...
    for step in getattr(construct, "steps", ()):
        for value in step.produces:
            found.append((f"steps.{step.id}.produces.{value.name}", value.type))
    return found
...
def _check_conflicting_obligations(
    ...
    for node in lowered.all_nodes():
        found = []
        if node.kind == "check" and node.phase and not node.checks:
            found.append((node.phase, node.command, node.prohibits, node.source))
        for invariant in node.checks:
            found.append(
                (node.phase, invariant.command, invariant.prohibits, invariant.source)
            )
...
def _check_protocol_data_flow(
    lowered: LoweredSkill, diagnostics: Diagnostics
) -> None:
    """Require non-optional protocol data to be assigned before a hook reads it.

    Optional fields always exist as possibly-absent values. Required fields may
    instead be established by an early hook, which is the only route open to a
    record, whose value has no literal form to declare as a default. Tracking
    lifecycle state together with assigned fields keeps conditional hooks and
    joins from turning a maybe-assignment into a definite one.
    """
    for frame in lowered.frames:
        _check_frame_data_flow(frame, lowered, diagnostics)


def _check_frame_data_flow(
    ...
    required = {
        declared.name
        for declared in protocol.data
        if declared.type.kind != "optional"
    }
    if not required:
        return
    initial = frozenset(
        declared.name
        for declared in protocol.data
        if declared.name in required and declared.defaulted
    )
    scope = (
    ...
    entry_steps = _frame_entries(frame, lowered)
    flows: dict[
        tuple[str, str], set[tuple[str, frozenset[str]]]
    ] = {}

    def merge(
        key: tuple[str, str], values: set[tuple[str, frozenset[str]]]
    ) -> bool:
        current = flows.setdefault(key, set())
        before = len(current)
        current.update(values)
        return len(current) != before

    for entry in entry_steps:
        merge(entry, {(protocol.initial, initial)})
    changed = True
    while changed:
        changed = False
        for item in lowered.workflows:
            ...
            for identifier in item.graph.order:
                ...
                incoming = flows.get(key, set())
                if not incoming:
                    continue
                outgoing = _apply_data_hooks(
                    protocol, nodes_by_step.get(key, []), incoming
                )
                step = steps[identifier]
                if step.form == "use" and frame.scope == "run":
                    callee = lowered.sources.workflows.get(step.call)
                    if callee is not None:
                        changed |= merge((callee.id, callee.entry), outgoing)
                        outgoing = _callee_data_exit(
                            callee, flows, nodes_by_step, protocol
                        )
                for target in step.successors:
                    changed |= merge((workflow.id, target), outgoing)

    hooks = {hook.id: hook for hook in protocol.hooks}
    reported: set[tuple[str, str, str]] = set()
    for key, nodes in sorted(nodes_by_step.items()):
        current = flows.get(key, set())
        for node in nodes:
            hook = hooks.get(node.hook)
            if hook is None:
                continue
            applicable = {
                pair for pair in current if pair[0] in set(hook.from_states)
            }
            for name in sorted(_hook_state_reads(hook) & required):
                if applicable and any(
                    name not in assigned for _, assigned in applicable
                ):
                    identity = (key[0], key[1], f"{hook.id}:{name}")
                    if identity in reported:
                        continue
                    diagnostics.error(
                        f"{protocol.path}: hook {hook.id} reads state.{name} "
                        f"at `{key[0]}.{key[1]}` on a path where no declared "
                        "default or earlier hook has assigned it",
                        "value.missing-binding",
                        protocol.path,
                    )
                    reported.add(identity)
            current = _apply_data_hooks(protocol, [node], current)


def _hook_state_reads(hook: Any) -> set[str]:
    found: set[str] = set()
    expressions = [hook.when]
    if hook.verify is not None and hook.verify.expression is not None:
        expressions.append(hook.verify.expression)
    for expression in expressions:
        if expression is None:
            continue
    ...
    for _, binding in hook.updates:
        ...
        if (
            ...
            found.add(str(reference.path[0]))
    return found


def _apply_data_hooks(
    protocol: Any,
    nodes: list[Node],
    incoming: set[tuple[str, frozenset[str]]],
    ...
    for node in nodes:
        ...
        if hook is None:
            continue
        condition = constant_boolean(hook.when) if hook.when is not None else True
        following: set[tuple[str, frozenset[str]]] = set()
        for state, assigned in current:
            if state not in hook.from_states or condition is False:
                following.add((state, assigned))
                continue
            updated = frozenset(
                set(assigned) | {name for name, _ in hook.updates}
            )
            applied = (hook.to or state, updated)
            following.add(applied)
...
def _callee_data_exit(
    callee: Any,
    flows: dict[tuple[str, str], set[tuple[str, frozenset[str]]]],
    nodes_by_step: dict[tuple[str, str], list[Node]],
    protocol: Any,
) -> set[tuple[str, frozenset[str]]]:
    exits: set[tuple[str, frozenset[str]]] = set()
    for step in callee.steps:
        if step.form != "return":
            continue
        key = (callee.id, step.id)
        exits.update(
            _apply_data_hooks(
                protocol, nodes_by_step.get(key, []), flows.get(key, set())
            )
        )
    return exits
...
def _check_frame(
    ...
    for _ in range(len(scope) + 2):
        changed = _propagate(frame, lowered, scope, entry_steps, nodes_by_step, states)
        if not changed:
            break
...
def _frame_entries(frame: Frame, lowered: LoweredSkill) -> list[tuple[str, str]]:
    """Every step this frame can be open at from the start, as workflow and step.

    A run frame has one for each entrypoint target, because a run enters
    whichever route matched the request and the frame opens there. All of them
    are seeded rather than one: a state reachable only through the second route
    would otherwise look unreachable, and a hook that reads it would be reported
    as an impossible transition on a source that is correct.

    Every other scope has exactly one, and the list is how they share a caller.
    """
    if frame.scope == "step":
        return [(frame.workflow, frame.step)]
    names = lowered.skill.roots if frame.scope == "run" else (frame.workflow,)
    found: list[tuple[str, str]] = []
    for name in names:
        entry = lowered.sources.workflows.get(name)
        if entry is not None:
            found.append((name, entry.entry))
    return found
...
def _propagate(
    ...
    scope: list[str],
    entry_steps: list[tuple[str, str]],
    nodes_by_step: dict[tuple[str, str], list[Node]],
    states: dict[tuple[str, str], frozenset[str]],
) -> bool:
    protocol = frame.protocol
...
def _result_dict(
    ...
    *,
    ...
    result: dict[str, Any] = {
        ...
        "source": skill.root,
        "entrypoints": _entrypoint_rows(skill),
        ...
        "lowering": _lowering_rows(lowered, rendered),
...
def _check_entrypoint_bindings(
    content: SkillContent, diagnostics: Diagnostics
) -> None:
    """Check each route's supplied values against the workflow it enters.

    An entrypoint is taken before any step runs, so there is no earlier value
    for `from` to read: every namespace DExpr offers is written by something
    that has not happened yet. A literal has no such problem, and it is what
    lets two routes enter one workflow with different values — which is how a
    skill offers several modes of one job without duplicating it.

    An input the route does not supply is not missing. It is an activation
    input: the agent establishes it from the request, which is what a root
    workflow's inputs have always been. So this checks what is supplied and
    says nothing about what is not.
    """
    skill = content.skill
    records = content.sources.record_types()
    path = skill.root / "skill.yaml"
    declared = skill.manifest.get("entrypoints")
    if not isinstance(declared, dict):
        return
    for route in skill.entrypoints:
        supplied = declared.get(route.id, {}).get("with")
        if supplied is None:
            continue
        where = f"entrypoints.{route.id}.with"
        if not isinstance(supplied, dict):
            diagnostics.error(
                f"{path}: {where} is a mapping of the target's inputs to the "
                "values this route supplies",
                "entrypoint.invalid-with",
                path,
            )
            continue
        target = content.sources.workflows.get(route.target)
        expected = dict(target.inputs) if target is not None else {}
        unknown = sorted(set(supplied) - set(expected))
        if unknown and target is not None:
            diagnostics.error(
                f"{path}: {where} supplies {', '.join(unknown)}, which workflow "
                f"{route.target} does not declare",
        ...
        for name, item in supplied.items():
            ...
            if binding.kind == "from":
                diagnostics.error(
                    f"{path}: {where}.{name} reads "
                    f"{binding.reference.render()}, which no step has produced "
                    "when a route is chosen; supply a literal, or leave the "
                    "input for the agent to establish from the request",
                    "entrypoint.invalid-with",
                    path,
                )
                continue
            expects = expected.get(name)
            if expects is None:
                continue
            for issue in check_binding(
                binding, expects, TypeEnvironment(values={}, records=records)
            ):
                diagnostics.error(
                    f"{path}: {where}.{name}: {issue.message}", issue.code, path
                )


def _entrypoint_rows(skill: Skill) -> list[dict[str, Any]]:
    """The routing table, in the order the manifest offers it.

    Order is the row's own content here rather than presentation: an agent takes
    the first route whose discriminator matches, and the last route is the one
    taken when none did, so a report that sorted these would describe a skill
    that behaves differently from the one on disk.
    """
    # Read the routes once. `Skill.entrypoints` builds a fresh tuple per call,
    # so asking it twice and comparing across the two would compare equal values
    # that are never the same object, and no row would be marked.
    routes = skill.entrypoints
    catch_all = len(routes) - 1 if routes and not routes[-1].when else -1
    return [
        {
            "id": item.id,
            "target": item.target,
            "when": item.when,
            "default": position == catch_all,
        }
        for position, item in enumerate(routes)
    ]
...
def _workflow_rows(compiled: Compiled) -> list[dict[str, Any]]:
    ...
    roots = set(compiled.content.skill.roots)
...
def _construct_nodes(lowered: LoweredSkill) -> dict[tuple[str, str, str], list[str]]:
    ...
    A check node carrying several items names none of them itself: each item on
    it is indexed here instead, so a report still points at the node that
    enforces one provision rather than at the whole placement.
    """
...
def _lowering_rows(
    lowered: LoweredSkill | None, rendered: RenderedBundle | None = None
) -> list[dict[str, Any]]:
    if lowered is None:
        return []
    rows: list[dict[str, Any]] = []
    ...
    for row in rows:
        row["bytes"] = (
            rendered.binding_bytes.get((row["kind"], row["id"]), 0)
            if rendered else 0
        )
    return rows
