...
class SkillContent:
    ...
    def icon_addresses(self) -> dict[str, str]:
        """Where each declared icon role lands, for whatever points at it."""
        return {
            role: self.addresses.icons[role]
            for role in sorted(self.icon_sources)
        }
...
def load_content(skill: Skill, diagnostics: Diagnostics) -> SkillContent:
    ...
    content.icon_sources, content.icon_assets = _load_icons(
        skill, content.addresses.icons, diagnostics
    )
    return content
...
def _claimed_addresses(content: SkillContent) -> set[str]:
    ...
    claimed = {ROOT, OPENAI_METADATA}
    for key in COPIED_CONTENT_KEYS:
        for path in content.files(key):
            claimed.add(copied_path(key, path.relative_to(root).as_posix()))
...
def _load_icons(
    skill: Skill, addresses: Mapping[str, str], diagnostics: Diagnostics
    ...
    try:
        ...
        assets = render_icon_assets(sources, addresses)
    ...
    for name in known:
        if name in used:
            ...
            if (
                ...
                diagnostics.warning(
                    f"{manifest}: every task names the principle {name!r}; name it "
                    "once at skill level instead; activation still controls when its "
                    "page is opened",
                    "principle.all-tasks",
                    manifest,
                )
            if task_paths and name in skill.principles:
                diagnostics.warning(
                    ...
                    "keep one placement matching its required reach",
        ...
        diagnostics.warning(
            ...
            "bundle carries none of it; selected principles must have an owner",
            "principle.unused",
            item.path,
        )
...
def _body_links(content: SkillContent) -> set[str]:
    """Every selected file an authored inline reference opens, by source path.

    A script or asset target is resolved exactly as the renderer resolves it,
    so a reference that opens a file on a page counts as using that file here.
    An ambiguous target opens nothing, and its own finding says so.
    """
    root = content.skill.root
    sources = content.sources
    copied = {
...
def _check_links(content: SkillContent, diagnostics: Diagnostics) -> None:
    ...
    writes its link from the page itself. Each finding is reported against the
    ...
    for body, path in _authored_bodies(content.sources):
        for target in dict.fromkeys(internal_links(body)):
            diagnostics.error(
                f"{path}: links {target!r} by path; link a task, principle, facet, "
                "guide, script, or asset with an inline reference such as "
...
def _generated_addresses(result: Compiled) -> set[str]:
    ...
    return {ROOT, OPENAI_METADATA, *pages, *result.content.icon_assets}
