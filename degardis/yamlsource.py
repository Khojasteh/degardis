"""Read one YAML source under the narrow subset this format accepts.

The format accepts mappings, lists, strings, integers, finite numbers, booleans,
...
something the subset refuses, is raised as a SourceError carrying the line and
...
The same reinterpretation threatens the YAML a build writes for a host, so the
one way an authored string is written into generated YAML lives here too.
"""
...
# The only tags this format's source may resolve to. Every other tag names a type
# the subset does not carry, whether the author wrote it or YAML's own implicit
...
class _Rejected(Exception):
    """One construct the subset refuses, at the line that wrote it."""
...
class StrictLoader(yaml.SafeLoader):
    """A safe loader narrowed to the values this format accepts.
...
def load_yaml_text(
    text: str, path: Path, *, line_offset: int = 0
) -> dict[str, Any]:
    """Read YAML already in hand, reporting failures against the file it came from.

    A Markdown source carries its fields in frontmatter, which is YAML held
    inside another file. The same subset has to apply to it — an alias, a merge
    key, or a repeated field changes what the compiler reads there exactly as it
    would in skill.yaml — and the reader has already separated the frontmatter
    from the body. The offset is how many lines of the file precede the block,
    so a reported line is the line of the file rather than of the fragment.

    An empty block loads as no fields rather than as a failure. The checks that
    read the fields report each missing one by name, which tells an author what
    to write; a bare "no mapping" would tell them only that something is wrong.
    """
    try:
        data = yaml.load(text, Loader=StrictLoader)
    except _Rejected as exc:
        line = None if exc.line is None else exc.line + line_offset
        raise _source_error(
            path, "source.rejected-yaml", f"rejected YAML: {exc.explanation}", line
        ) from exc
        ...
        raise _source_error(
            path,
            "source.invalid-yaml",
            f"invalid YAML: {explanation}",
            None if line is None else line + line_offset,
        ) from exc
    if data is None:
        return {}
    if not isinstance(data, dict):
        raise SourceError(
            f"{path}: invalid YAML: the frontmatter holds no mapping of fields",
            "source.invalid-yaml",
            path,
        )
    return data
...
def read_source_text(path: Path) -> str:
    """Read one source file, under the check an unreadable source is reported by.

    A byte-order mark is dropped rather than read as text. Some Windows editors
    and shells open every UTF-8 file with one, and it is invisible to the author
    whose file then seems to open with its frontmatter fence and does not.
    """
    try:
...
# What JSON leaves as a raw character but a YAML reader would not read back as
# itself: C1 controls and DEL, which YAML does not print; the line and paragraph
# separators and NEL, which YAML 1.1 reads as line breaks inside a quoted value;
# and the byte-order mark and the two non-characters.
_YAML_UNPRINTABLE = re.compile(
    "[\x7f-\x9f" + "".join(map(chr, (0x2028, 0x2029, 0xFEFF, 0xFFFE, 0xFFFF))) + "]"
)


def yaml_string(value: str) -> str:
    """One string as a double-quoted YAML scalar that reads back as itself.

    Generated host metadata is parsed by YAML readers this compiler does not
    choose, so an authored value is never written plain: a colon and a space in
    a description would start a mapping, a ` #` would cut it short, and a quoted
    `1.10` would come back as a number. JSON's string syntax is a subset of
    YAML's double-quoted scalar, and every character is written as itself
    rather than escaped, because a JSON escape for a character outside the
    basic plane is a surrogate pair that libyaml refuses. What remains is
    escaped explicitly, as a code point YAML accepts.
    """
    quoted = json.dumps(value, ensure_ascii=False)
    return _YAML_UNPRINTABLE.sub(lambda match: f"\\u{ord(match.group()):04x}", quoted)
...
def _plain_value_warnings(
    path: Path, token: yaml.ScalarToken, line_offset: int = 0
) -> list[Diagnostic]:
...
def yaml_scalar_warnings(
    path: Path, text: str | None = None, line_offset: int = 0
) -> list[Diagnostic]:
    ...
    `text` is the block to scan, for a source whose YAML is frontmatter inside a
    Markdown file: scanning the whole file there would hand the scanner a body
    that is not YAML at all. `line_offset` is how many lines precede that block,
    so the warning names the line of the file.
    """
    if text is None:
        try:
            text = read_source_text(path)
    ...
    for index, token in enumerate(tokens):
        ...
        if isinstance(value_token, yaml.ScalarToken) and value_token.plain:
            warnings.extend(
                _plain_value_warnings(path, value_token, line_offset)
            )
    return warnings
