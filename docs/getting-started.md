...
Install Degardis, build the worked example, then start a skill of your own. This
page is the first run; the [Manual](manual.md) is the reference for every
construct, field, and rule, and `degardis manual TOPIC` prints the same topics
at a terminal.
...
`--zip` for an archive a host can accept as an upload. A build runs those same checks itself and writes
nothing unless they pass, so the separate `validate` step here is for reading the
...
as separately loaded pages when the root or task links to them.

Treat the folder as output: change the source and build again rather than
editing inside it. [Bundles](artifact-format.md) describes what a bundle
contains and what a rebuild replaces.
...
the skill or task that needs them, and facets carry what depends on the situation
...
carry this material? — by naming the knowledge the task asked for, the knowledge
that came in through `requires`, and the order each kind renders in.
`principles` names each principle's file, the levels that name it, and its own
