...
COMMANDS = ("list", "validate", "build", "inspect", "explain", "manual")
...
class ParserTests(unittest.TestCase):
    ...
    def test_every_help_text_naming_the_format_reads_the_current_version(self):
        ...
        for argv in ([], ["validate"], ["inspect"], ["manual"]):
    ...
    def test_the_inspect_help_names_the_commands_an_agent_needs_next(self):
        text = help_text("inspect")
        for command in ("degardis explain", "degardis validate", "degardis build", "degardis manual"):
            with self.subTest(command=command):
                self.assertIn(command, text)
...
class ListTests(unittest.TestCase):
    ...
    def test_list_marks_the_route_taken_when_nothing_else_matched(self):
        """A reader has to see which route a request falls through to.

        Nothing in the manifest marks it — the route is the catch-all by being
        written last with no condition — so the report is where that fact
        becomes visible, and it is worth its own case because a report that
        marked no route at all would still name every one of them.
        """
        status, report, _ = run("list", str(FIXTURES))
        self.assertEqual(0, status)
        self.assertIn("run (default)", report)
...
class InspectTests(unittest.TestCase):
    ...
    def test_a_profile_row_counts_what_it_contributes(self):
        ...
        source use one vocabulary: `points` for the prose, `references` for the
        pages it links."""
        _, described, _ = run(
            "inspect", str(FIXTURES / "alpha"), "--only", "profiles"
        )
        self.assertIn("points=2 | references=1", described)
