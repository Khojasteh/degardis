...
from degardis.inspection import INSPECT_DIMENSIONS, inspect_skills
...
class InspectTests(unittest.TestCase):
    ...
    def test_the_size_row_states_each_page_against_its_budget(self):
        ...
        self.assertIn(
            ...
            f"{attention['largest_guide_bytes']}B/{GUIDE_BUDGET_BYTES}B",
            report,
        )
        self.assertIn(
            f"facets {attention['facet_bytes']}B max "
            f"{attention['largest_facet_bytes']}B/"
            f"{FACET_BUDGET_BYTES}B",
            report,
        )

    def test_the_size_row_separates_principle_and_facet_pages(self):
        result = inspect_skills([FIXTURES / "alpha"])[0]
        ...
        facets = sum(
            size
            for path, size in sizes.items()
            if path.startswith("references/facets/")
        )

        _, report, _ = run("inspect", str(FIXTURES / "alpha"))

        self.assertIn(f"| principles {principles}B", report)
        self.assertIn(f"| facets {facets}B", report)
    ...
    def test_the_guides_dimension_reports_each_guide_condition(self):
        ...
        self.assertIn("guides 2", report)
        self.assertIn("checklist", report)
        self.assertIn("guides/checklist.md", report)
        condition = field_of(FIXTURES / "alpha" / "guides" / "checklist.md", "activation")
        self.assertIn(f"{condition} -> task:review, facet:legacy", report)

    def test_the_guides_dimension_qualifies_a_task_owner_and_a_facet_owner(self):
        """A guide reached only from a facet has an owner, and the row says so.

        Unqualified ids would leave a facet-owned guide reading as one no task
        names, which is how a guide nothing uses reads.
        """
        _, report, _ = run("inspect", str(FIXTURES / "alpha"), "--only", "guides")
        self.assertIn("-> facet:python", report)
