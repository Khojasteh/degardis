...
class DiagnosticIdentityTests(unittest.TestCase):
    ...
    def finding(self, **fields: object) -> Diagnostic:
        record: dict[str, object] = {
            "severity": "error",
            "message": "alpha: knowledge/unused.md is reached by no task",
            "code": "knowledge.orphan",
            "path": Path("knowledge/unused.md"),
            "line": 4,
        }
    ...
    def test_findings_differing_in_any_identity_field_are_both_kept(self):
        ...
        for field, other in (
            ("severity", "warning"),
            ("message", "alpha: knowledge/unused.md is carried by no page"),
            ("code", "knowledge.empty"),
            ("path", Path("knowledge/spare.md")),
            ("line", 9),
        ):
    ...
    def test_a_finding_carries_its_code_when_rendered_as_one_string(self):
        ...
        self.assertTrue(rendered.endswith("(knowledge.orphan)"))
        self.assertEqual(finding.message, rendered.removesuffix(" (knowledge.orphan)"))
...
class CollectedFailureTests(unittest.TestCase):
    ...
    def test_a_clean_collector_raises_nothing(self):
        collector = Diagnostics()
        collector.warning("alpha: no task reaches unused", "knowledge.orphan")

        self.assertIsNone(collector.raise_if_errors())

    def test_one_collected_error_is_raised_carrying_its_check_code(self):
        collector = Diagnostics()
        collector.error("alpha: task review names nowhere", "task.unknown-knowledge")

        with self.assertRaises(DegardisError) as raised:
            collector.raise_if_errors()

        self.assertEqual("task.unknown-knowledge", raised.exception.code)
    ...
    def test_several_collected_errors_each_name_their_own_check(self):
        ...
        collector = Diagnostics()
        collector.error("alpha: task review names nowhere", "task.unknown-knowledge")
        collector.error("alpha: principles names clear-reporting", "manifest.unknown-principle")
        ...
        for line, code in zip(lines[1:], ("task.unknown-knowledge", "manifest.unknown-principle")):
...
class SourceFailureTests(unittest.TestCase):
    ...
    def test_a_source_error_keeps_its_own_check_and_line(self):
        collector = Diagnostics()
        failure = SourceError(
            "knowledge/unused.md:4: knowledge unused: title is required",
            "knowledge.missing-title",
            Path("knowledge/unused.md"),
            4,
        )
        ...
        self.assertEqual("knowledge.missing-title", record.code)
        self.assertEqual(Path("knowledge/unused.md"), record.path)
