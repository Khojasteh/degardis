...
from degardis.model import CURRENT_FORMAT_VERSION, DegardisError
...
class DiscoveryTests(unittest.TestCase):
    ...
    def test_a_bundle_built_inside_a_collection_is_passed_over(self):
        """A build into a directory the collection contains leaves output that a
        later run over the collection has to walk past, not stop at."""
        collection = copy_skills(self.root)
        write_text(collection / "dist" / "alpha" / "SKILL.md", "# Built\n")
        write_text(collection / "dist" / "alpha" / "assets" / "skill.yaml", "name: x\n")
        found = discover_skill_paths([collection])
        self.assertEqual(["alpha", "beta"], [path.name for path in found])
        self.assertNotIn("dist", {part for path in found for part in path.parts})
...
class ManifestIdentityTests(unittest.TestCase):
    ...
    def test_a_manifest_is_loaded_with_its_identity(self):
        ...
        self.assertEqual(CURRENT_FORMAT_VERSION, skill.format_version)
    ...
    def test_the_manifest_names_the_skill_whatever_its_directory_is_called(self):
        (self.root / "alpha").rename(self.root / "alpha-source")
        skill = load_skill_path(self.root / "alpha-source")
        self.assertEqual("alpha", skill.name)
