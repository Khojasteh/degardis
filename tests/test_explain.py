...
class ExplainDataTests(unittest.TestCase):
    ...
    def test_the_directory_listing_is_the_vocabulary(self):
        ...
        self.assertEqual(
            [],
            [path.name for path in self.codes_dir().iterdir() if path.suffix != ".yaml"],
        ...
        for code, entry in checks().items():
            for label in ("trigger", "impact", "failing", "passing"):
                with self.subTest(code=code, label=label):
                    self.assertNotIn("{{", getattr(entry, label))

    def test_an_example_states_its_lines_as_lines(self):
        r"""An example is source an author copies, so a \n in it is not a newline.

        Written as text rather than as a line break, it reaches the reader as
        two visible characters in the middle of the source they are copying.
        """
        for code, entry in checks().items():
    ...
    def test_a_shared_wording_carries_the_values_its_code_file_supplies(self):
        ...
        for path in sorted(self.codes_dir().glob("*.yaml")):
            ...
            for name, value in data.items():
                if name in ("wording", "failing", "passing"):
                    continue
    ...
    def test_an_example_naming_the_format_version_tracks_the_one_enforced(self):
        """A printed example must not drift from the format `validate` accepts."""
        ...
        self.assertIn("format_version: 998", obsolete.failing)
        self.assertIn("format_version: 999", obsolete.passing)
        self.assertIn("reads (999)", obsolete.trigger)
        self.assertIn("format_version: 1000", unsupported.failing)
