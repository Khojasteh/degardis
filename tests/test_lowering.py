...
class DataFlowTests(unittest.TestCase):
    ...
    def test_nodes_render_actual_reads_not_all_live_values(self):
        ...
        # The gate's heuristic tests exists(result.finding).
        self.assertEqual(("result.finding",), authorize.available)
        self.assertNotIn("input.wide", authorize.available)
...
class ControlFlowTests(unittest.TestCase):
    ...
    def test_before_and_after_obligations_keep_their_boundary_order(self):
        ...
        after = next(
            node for node in workflow.nodes
            if node.step == "apply-narrow" and node.phase == "after"
            and any(item.local == "report-effect" for item in node.checks)
        )
...
class CheckGroupingTests(unittest.TestCase):
    """What the compiler does with several checks that meet at one boundary.

    Each provision is read separately, which is the only reason each became a
    node of its own. An agent reaching one decision through eighteen of them
    reads eighteen headings, eighteen labels, and eighteen edge pairs for one
    boundary whose items all come from one construct at one phase. So they are
    one node listing its items, which is the shape a `during` item already
    renders in, and a lone check keeps the shape it has: its own sentence is a
    better heading than any sentence the compiler could write over it.
    """
    ...
    def setUp(self) -> None:
        ...
        self.policy = self.skill / "policies" / "workflow-scope.yaml"

    def declared(self) -> dict:
        """The provisions the fixture's own policy file states, in order."""
        with edit_yaml(self.policy) as data:
            pass
        return data["provisions"]

    def add(self, **provisions: dict) -> None:
        with edit_yaml(self.policy) as data:
            data["provisions"].update(provisions)

    def checks(self, construct: str = "workflow-scope", phase: str = "before") -> list:
        _, result, diagnostics = compiled(self.skill)
        self.assertEqual(
            [], [item.message for item in diagnostics.records if item.severity == "error"]
        )
        self.rendered = result.rendered
        self.lowered = result.lowered
        return [
            node
            for node in result.lowered.all_nodes()
            if node.kind == "check"
            and node.origin == f"policy:{construct}"
            and node.phase == phase
        ]

    def test_one_provision_at_a_boundary_states_its_own_command(self):
        declared = self.declared()
        self.assertEqual(1, len(declared))
        (node,) = self.checks()
        self.assertEqual(
            declared["stay-on-subject"]["require"].replace("\n", " "),
            node.command,
        )
        self.assertEqual((), node.checks)
        self.assertEqual("stay-on-subject", node.provision)

    def test_several_provisions_of_one_policy_at_one_boundary_are_one_node(self):
        self.add(
            **{
                "name-the-reader": {
                    "phase": "before",
                    "match": {"subjects": ["report.write"]},
                    "require": "Name the reader this report is written for.",
                },
                "no-second-subject": {
                    "phase": "before",
                    "match": {"subjects": ["report.write"]},
                    "prohibit": "Carry a second subject into the report.",
                    "when": "input.wide == true",
                    "verify": {"gate": "authorization"},
                },
            }
        )
        declared = self.declared()
        ...
        self.assertEqual(
            [
                (body.get("require") or body["prohibit"]).replace("\n", " ")
                for body in declared.values()
            ],
            [item.command for item in node.checks],
        )
        # Each item keeps its own obligation, activation, and proof, because a
        # group states one heading and the items are what differ under it.
        self.assertEqual([False, False, True], [item.prohibits for item in node.checks])
        activation = [bool(item.activation) for item in node.checks]
        self.assertEqual([False, False, True], activation)
        self.assertEqual([False, False, True], [bool(item.verify) for item in node.checks])

    def test_a_grouped_prohibition_keeps_the_sentence_the_source_wrote(self):
        prohibition = "Carry a second subject into the report."
        self.add(
            **{
                "no-second-subject": {
                    "phase": "before",
        ...
        (node,) = self.checks()
        text = "\n".join(self.rendered.execution_modules.values())
        block = next(
            part for part in re.split(r"(?m)(?=^### )", text)
            if part.startswith(f"### [`{node.label}`]")
        )
        self.assertIn(f"**{wording.PROHIBITED}** {prohibition}", block)
        self.assertNotIn(prohibition, block.splitlines()[0])

    def test_a_policy_and_a_rule_at_one_boundary_stay_separate_nodes(self):
        """Grouping is per construct, so provenance survives it.

        One node carries one construct's items, which is what lets a report say
        which construct is speaking and what lets the bytes it costs be charged
        to it.
        """
        self.checks()
        at_change = [
            node
            for node in self.lowered.all_nodes()
            if node.kind == "check" and node.step == "apply-narrow" and node.phase == "before"
        ]
        self.assertEqual(
            {"policy:run-authority", "rule:scoped-change"},
            {node.origin for node in at_change},
        )

    def test_each_grouped_provision_is_attributed_to_the_node_it_reached(self):
        self.add(
            **{
                "name-the-reader": {
                    "phase": "before",
        ...
        (node,) = self.checks()
        rows = {row["id"]: row for row in inspect_one(self.skill)["lowering"]}
        for identifier in ("workflow-scope.stay-on-subject", "workflow-scope.name-the-reader"):
            with self.subTest(provision=identifier):
                self.assertEqual([node.label], rows[identifier]["nodes"])
                self.assertTrue(rows[identifier]["lowered"])
                self.assertGreater(rows[identifier]["bytes"], 0)
...
class DuringInvariantTests(unittest.TestCase):
    """What a `during` provision carries to the command it renders beside.

    A `during` item generates no check node of its own, so its activation
    condition and its verification have nowhere to go but the node its selector
    matched. An item that arrived there carrying only its sentence would leave
    the proof the author declared out of the run, and out of every check that
    reads one: the gate a verification names would go unchecked at this phase
    while the identical text is refused at every other.
    """

    # Written as the source states them, so a test that passes proves the item
    # reached the node rather than that the renderer holds these strings.
    CONFIRMATION = "Every effect names the authority established for it."
    CONDITION = "input.wide == true"
    ...
    def provision(self, **fields: object) -> None:
        """Add fields to the fixture's own `during` provision."""
        with edit_yaml(self.skill / "policies" / "run-authority.yaml") as data:
            data["provisions"]["no-inferred-benefit"].update(fields)
    ...
    def invariants(self) -> list:
        ...
        return [
            ...
            if invariant.construct == "run-authority"
            and invariant.local == "no-inferred-benefit"
        ]

    def test_a_verification_reaches_every_node_the_item_renders_on(self):
        self.provision(verify={"confirm": self.CONFIRMATION})
        found = self.invariants()
        self.assertTrue(found)
        for invariant in found:
            with self.subTest(command=invariant.command):
                self.assertEqual(self.CONFIRMATION, invariant.verify)

    def test_an_activation_condition_reaches_every_node_it_conditions(self):
        self.provision(when=self.CONDITION)
        found = self.invariants()
        self.assertTrue(found)
        for invariant in found:
            with self.subTest(command=invariant.command):
                self.assertIn(self.CONDITION, invariant.activation)

    def test_a_gate_verification_must_dominate_the_command_it_proves(self):
        """The gate the verification reads has to be on every path to the node.

        `authorization` is the fixture's own gate and does dominate the change
        the provision constrains; a name no step declares cannot, so the phase
        that renders the verification is also the phase that has to refuse it.
        """
        self.provision(verify={"gate": "authorization"})
        self.assertEqual(set(), codes(self.skill))
        self.provision(verify={"gate": "no-such-gate"})
        self.assertIn("workflow.missing-gate", codes(self.skill))

    def test_a_verification_may_not_name_a_heuristic(self):
        self.provision(verify={"gate": "smallest-change"})
        self.assertIn("heuristic.used-as-authority", codes(self.skill))

    def test_an_activation_condition_is_checked_against_the_values_in_scope(self):
        self.provision(when="result.nothere == true")
