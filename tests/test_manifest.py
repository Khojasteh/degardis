...
class ManifestFieldTests(unittest.TestCase):
    ...
    def test_a_required_field_left_out_names_the_key_it_is_missing(self):
        ...
        for field, code in (
            ...
            ("entrypoints", "manifest.missing-entrypoints"),
    ...
    def test_each_string_and_mapping_field_names_its_own_invalid_code(self):
        cases = (
            ...
            ("entrypoints", [], "manifest.invalid-entrypoints"),
    ...
    def test_a_binding_list_must_be_a_list_of_stems(self):
        expected = {
            "policies": "manifest.invalid-policies",
            "rules": "manifest.invalid-rules",
            "protocols": "manifest.invalid-protocols",
            "guidance": "manifest.invalid-guidance",
        }
        for key in MANIFEST_BINDING_KEYS:
            with self.subTest(key=key), self.manifest() as data:
                data[key] = "one-thing"
            self.assertIn(expected[key], codes(self.skill))
            self.setUp()
    ...
    def test_a_name_that_is_not_an_identifier_is_reported(self):
        with self.manifest() as data:
            data["entrypoints"] = {"run": {"target": "Run Me"}}
        self.assertIn("entrypoint.invalid-target", codes(self.skill))
    ...
    def test_an_entrypoint_field_the_schema_does_not_declare_is_reported(self):
        """`default` is the one worth naming: position says which route it is."""
        with self.manifest() as data:
            data["entrypoints"] = {"run": {"default": True, "target": "run"}}
        self.assertIn("entrypoint.unknown-field", codes(self.skill))

    def test_an_entrypoint_with_no_target_is_reported(self):
        """A route that states when it applies but not where it goes."""
        with self.manifest() as data:
            data["entrypoints"] = {"run": {"when": "The request asks for it."}}
        self.assertIn("entrypoint.missing-target", codes(self.skill))

    def test_a_route_with_no_condition_before_another_route_is_reported(self):
        """An unconditional route is always taken, so nothing may follow it."""
        with self.manifest() as data:
            data["entrypoints"] = {
                "run": {"target": "run"},
                "other": {"when": "The request asks for it.", "target": "run"},
            }
        self.assertIn("entrypoint.missing-when", codes(self.skill))

    def test_a_route_with_no_condition_written_last_is_accepted(self):
        """The catch-all is declared by being last, and needs no other mark."""
        with self.manifest() as data:
            data["entrypoints"] = {
                "other": {"when": "The request asks for it.", "target": "run"},
                "run": {"target": "run"},
    ...
    def test_every_route_stating_a_condition_is_accepted(self):
        """A skill may recognize nothing in a request, and decline it."""
        with self.manifest() as data:
            data["entrypoints"] = {
                "run": {"when": "The request asks for the task.", "target": "run"}
            }
        self.assertEqual(set(), codes(self.skill))

    def test_a_condition_that_is_not_a_sentence_is_reported(self):
        with self.manifest() as data:
            data["entrypoints"] = {
                "run": {"when": "", "target": "run"},
                "rest": {"target": "run"},
            }
        self.assertIn("entrypoint.invalid-when", codes(self.skill))

    def test_an_entrypoint_target_nothing_selects_is_reported(self):
        with self.manifest() as data:
            data["entrypoints"] = {"run": {"target": "absent"}}
        self.assertIn("source.unknown-reference", codes(self.skill))

    def test_a_route_reading_an_earlier_value_is_reported(self):
        """Nothing has run when a route is chosen, so there is no value to read."""
        with self.manifest() as data:
            data["entrypoints"] = {
                "run": {"target": "run", "with": {"request": {"from": "result.x"}}}
            }
        self.assertIn("entrypoint.invalid-with", codes(self.skill))

    def test_a_route_supplying_a_literal_its_target_declares_is_accepted(self):
        """Two routes enter one workflow by fixing the input that tells them apart."""
        with self.manifest() as data:
            data["entrypoints"] = {
                "run": {"target": "run", "with": {"request": {"literal": "audit"}}}
            }
        self.assertEqual(set(), codes(self.skill))

    def test_a_route_supplying_a_value_its_target_does_not_declare_is_reported(self):
        with self.manifest() as data:
            data["entrypoints"] = {
                "run": {"target": "run", "with": {"nonesuch": {"literal": "x"}}}
            }
        self.assertIn("value.unknown-binding", codes(self.skill))

    def test_a_route_supplying_a_literal_of_the_wrong_type_is_reported(self):
        with self.manifest() as data:
            data["entrypoints"] = {
                "run": {"target": "run", "with": {"request": {"literal": 12}}}
            }
        self.assertIn("value.mistyped-binding", codes(self.skill))

    def test_an_input_no_route_supplies_is_left_to_the_agent(self):
        """A root workflow's inputs are what the agent reads from the request.

        Nothing supplies them today either; this states that leaving one out is
        the ordinary case rather than an omission the compiler should report.
        """
        with self.manifest() as data:
            data["entrypoints"] = {"run": {"target": "run"}}
        self.assertEqual(set(), codes(self.skill))
...
class ContentSelectionTests(unittest.TestCase):
    ...
    def test_a_pattern_list_of_the_wrong_shape_is_reported(self):
        ...
        self.assertIn("content.invalid-rules", codes(self.skill))

    def test_each_content_field_names_its_own_invalid_code(self):
        expected = {
            "workflows": "content.invalid-workflows",
            "policies": "content.invalid-policies",
            "rules": "content.invalid-rules",
            "protocols": "content.invalid-protocols",
            "patterns": "content.invalid-patterns",
            "heuristics": "content.invalid-heuristics",
            "guidance": "content.invalid-guidance",
            "records": "content.invalid-records",
            "profiles": "content.invalid-profiles",
            "references": "content.invalid-references",
