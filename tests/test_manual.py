...
class ManualCommandTests(unittest.TestCase):
    ...
    def test_multiple_topics_print_once_in_request_order(self):
        ...
        self.assertEqual((0, ""), (status, stderr))
        self.assertEqual(1, stdout.count("# What you write\n"))
        self.assertLess(stdout.index("# What you write\n"), stdout.index("# YAML"))
        self.assertIn("| `recognize` | yes |", stdout)
    ...
    def test_topic_relationships_are_listed_after_the_topic_body(self):
        """A topic's catalog entry is the one place its related topics appear."""
        for topic in TOPICS:
            with self.subTest(topic=topic.name):
