...
def loaded_cost(result) -> tuple[int, int, int, int]:
    ...
    # Every root, not the first: a run enters whichever entrypoint matched the
    # request, so the cost a reader can be charged is the worst over all of
    # them. Enumerating one would let a layout make another route arbitrarily
    # expensive and still agree with what the renderer reported.
    routes = [
        row
        for root in result.lowered.skill.roots
        for row in walk(workflows[root].entry)
    ]
