...
from degardis.sources import PRINCIPLE_FIELDS
...
class OwnershipTests(unittest.TestCase):
    ...
    def test_the_compiler_package_ships_no_principle_of_its_own(self):
        ...
        self.assertFalse((package / "principles").exists())
        self.assertFalse((package / "principle_corpus").exists())
        self.assertFalse((package / "principles.py").exists())
    ...
    def test_one_source_states_one_thing_whichever_compiler_built_it(self):
        """Nothing outside the source contributes principle text, so an upgrade
        cannot change what a skill says."""
        from degardis import fingerprint

        text = (REPO_ROOT / "degardis" / "fingerprint.py").read_text(
            encoding="utf-8"
        )
        self.assertNotIn("corpus", text)
        self.assertFalse(hasattr(fingerprint, "compiler_corpus_digest"))
        page = pages(alpha(self.root))[principle_path("evidence")]
        source = (alpha(self.root) / "principles" / "evidence.md").read_text(
            encoding="utf-8"
        )
...
class SchemaTests(unittest.TestCase):
    ...
    def test_a_principle_states_a_title_and_may_state_an_activation(self):
        self.assertEqual({"title", "activation"}, set(PRINCIPLE_FIELDS))
...
class InspectionTests(unittest.TestCase):
    ...
    or profiles: a construct with its own verb reads as a special case, and this
    one is not.
    """
