...
from tests.support import CANONICAL_EXAMPLE, FIXTURES, Terminal
...
class StatusLineTests(unittest.TestCase):
    ...
    def test_a_stream_that_fails_mid_run_does_not_fail_the_run(self):
        """Completing is the whole assertion: the line is a courtesy, and a
        terminal that goes away must not take the compilation with it."""
    ...
    def test_a_displayed_run_and_a_silent_run_compile_the_same_bundle(self):
        ...
        for root in (CANONICAL_EXAMPLE, FIXTURES / "alpha"):
            with self.subTest(skill=root.name):
                ...
                self.assertEqual(
                    quiet.compiled.rendered.pages,
                    shown.compiled.rendered.pages,
                )
                self.assertEqual(
