...
from degardis.render import (
    MARKDOWN_LINK, MODULE_BUDGET_BYTES, ROOT_BUDGET_BYTES, profile_page,
)
...
def stated_heading(path: Path) -> str:
    """The name a document gives itself, read from that document.

    A link to supporting Markdown is named by the heading its target states, so
    the expectation belongs to the target file rather than to the renderer that
    reads it. The fixtures state theirs on the first line, which is what lets
    this stay a read of the source instead of a second implementation of the
    scan the compiler performs.
    """
    first = path.read_text(encoding="utf-8").splitlines()[0]
    if not first.startswith("# "):
        raise AssertionError(f"{path} states no heading on its first line")
...
class RootTests(unittest.TestCase):
    ...
    def test_start_names_an_exact_module_entry_and_complete_command(self):
        ...
        """
        _, entry = entry_of(self.result, self.skill.roots[0])
    ...
    def test_a_profile_links_its_references_rather_than_absorbing_them(self):
        """A profile names supporting Markdown the way every construct does.

        The link is what leaves the reference its own document and makes the
        profile page a route to it. A profile page sits one directory below the
        bundle root, so the link has to pass Markdown's own test: resolved
        against the page carrying it, it names the file the bundle ships.
        """
        linked = False
        for identifier, profile in self.sources.profiles.items():
            page = profile_page(profile, self.skill.root)
            text = self.rendered.pages[page]
            for target in profile.references:
                linked = True
                with self.subTest(profile=identifier, target=target):
                    self.assertIn(f"## {wording.PAGE_FURTHER}", text)
                    self.assertIn(
                        target,
                        [
                            posixpath.normpath(
                                posixpath.join(
                                    posixpath.dirname(page), match.group("target")
                                )
                            )
                            for match in MARKDOWN_LINK.finditer(text)
                        ],
                    )
                    body = (self.skill.root / target).read_text(encoding="utf-8")
                    self.assertNotIn(body.strip(), text)
        self.assertTrue(linked, "no profile names a reference, so nothing is tested")
...
class DuringInvariantRenderTests(unittest.TestCase):
    """Where a `during` provision's proof renders: against the command it bounds.

    An invariant has no node of its own, so its verification sits against the
    command it proves. Anywhere else, and the agent performing that command has
    no statement of what would establish it; nowhere at all, and the author's
    proof is in no installed page.
    """

    CONFIRMATION = "Every effect names the authority established for it."

    def rendered_lines(self, **fields: object) -> tuple[list[str], str]:
        """Render the fixture with these fields on its `during` provision."""
        directory = tempfile.TemporaryDirectory()
        self.addCleanup(directory.cleanup)
        skill = copy_skills(Path(directory.name)) / "alpha"
        with edit_yaml(skill / "policies" / "run-authority.yaml") as data:
            provision = data["provisions"]["no-inferred-benefit"]
            provision.update(fields)
            command = provision["prohibit"]
        _, result, _ = compiled(skill)
        text = "\n".join(result.rendered.execution_modules.values())
        return text.splitlines(), command

    def test_a_verification_renders_on_the_line_below_the_command(self):
        lines, command = self.rendered_lines(verify={"confirm": self.CONFIRMATION})
        stated = [
            index
            for index, line in enumerate(lines)
            if line == f"**{wording.PROHIBITED}** {command}"
        ]
        self.assertTrue(stated)
        for index in stated:
            with self.subTest(line=index):
                self.assertEqual(
                    f"**{wording.VERIFY}** {self.CONFIRMATION}", lines[index + 1]
                )

    def test_an_activation_condition_renders_below_the_command(self):
        lines, command = self.rendered_lines(when="input.wide == true")
        stated = [
            index
            for index, line in enumerate(lines)
            if line == f"**{wording.PROHIBITED}** {command}"
        ...
        for index in stated:
            with self.subTest(line=index):
                self.assertTrue(lines[index + 1].startswith(f"**{wording.ACTIVE}**"))
                self.assertIn("input.wide == true", lines[index + 1])
...
class ExecutionModuleTests(unittest.TestCase):
    ...
    def test_debug_provenance_is_not_paid_in_runtime_markdown(self):
        ...
        This holds for a bound item as much as for the node carrying it. The
        construct and provision an invariant came from are how `inspect` names
        the item an author repairs; the module states the command the agent
        performs. Rendering the citation beside the command charges it to every
        placement and gives the run nothing it can act on.
        """
        for node in self.result.lowered.all_nodes():
            ...
            for invariant in node.invariants:
                with self.subTest(node=node.label, binding=invariant.source):
                    self.assertNotIn(invariant.source, self.all_text)
...
class AdvisoryPageTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.skill, cls.result, _ = compiled(ALPHA)
        cls.sources = cls.result.lowered.sources
        cls.constructs = {
            **{
                f"references/{kind}/{identifier}.md": construct
                for kind in ("patterns", "heuristics", "guidance")
                for identifier, construct in cls.sources.kind(kind).items()
            },
            **{
                profile_page(profile, cls.skill.root): profile
                for profile in cls.sources.profiles.values()
            },
        }
    ...
    def test_a_link_to_supporting_markdown_is_named_by_that_document(self):
        """A link an agent reads has to say what it opens, not where it sits.

        Naming the link by its own path leaves the agent deciding whether to
        follow it from a directory and a filename. The document states what it
        is in its heading, and the profile index already names a page that way,
        so a reference is named by the heading of the file the link resolves to.
        """
        root = self.skill.root
        named = False
        for page, text in self.result.rendered.pages.items():
            construct = self.constructs.get(page)
            for target in getattr(construct, "references", ()):
                ...
                with self.subTest(page=page, target=target):
                    ...
                    self.assertEqual(
                        stated_heading(root / target), link.group("text")
                    )
        self.assertTrue(named, "no page links a reference, so nothing is tested")

    def test_a_node_names_the_page_its_further_reading_opens(self):
        """One rule holds wherever the generated text links a generated page.

        A node carries a pattern's procedure and a heuristic's advice as blocks
        with nothing beside them naming the construct, so the link is all the
        agent has to decide with, and an identifier is not what the page it
        opens is called.
        """
        pages = self.result.rendered.pages
        field = f"**{wording.PAGE_FURTHER}:**"
        named = False
        for path, module in self.result.rendered.execution_modules.items():
            for line in module.splitlines():
                ...
                for match in MARKDOWN_LINK.finditer(line):
                    target = posixpath.normpath(
                        posixpath.join(
                            posixpath.dirname(path), match.group("target")
                        )
                    )
                    named = True
                    with self.subTest(module=path, target=target):
                        heading = pages[target].splitlines()[0]
                        self.assertTrue(heading.startswith("# "), heading)
                        self.assertEqual(heading[2:].strip(), match.group("text"))
        self.assertTrue(named, "no node links a page, so nothing is tested")
