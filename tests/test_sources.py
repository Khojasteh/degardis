...
class ReaderHarness(unittest.TestCase):
    ...
    def profile(self, fields: str, body: str = "Body text."):
        return self.read(read_profile, "profiles/python.md", fields, body)
...
class KnowledgeReaderTests(ReaderHarness):
    ...
    def test_knowledge_kinds_have_one_canonical_render_order(self):
        self.assertEqual(
...
class ProfileReaderTests(ReaderHarness):
    def test_a_profile_is_read_with_its_optional_fields(self):
        profile, found = self.profile(
            "title: Python\ncategory: Language\ndescription: Python guidance.\n"
        )
        self.assertEqual(set(), found)
        self.assertEqual(("python", "Python", "Language"), (profile.id, profile.title, profile.category))
        self.assertEqual("Python guidance.", profile.description)

    def test_a_profile_needs_only_a_title(self):
        profile, found = self.profile("title: Python\n")
        self.assertEqual(set(), found)
        self.assertEqual("", profile.description)

    def test_a_profile_keeps_prefixed_custom_metadata_without_using_it(self):
        profile, found = self.profile(
    ...
    def test_a_profile_declaring_an_id_is_refused(self):
        _, found = self.profile("title: Python\nid: py\n")
        self.assertIn("profile.unexpected-id", found)

    def test_a_profile_with_no_title_is_reported(self):
        _, found = self.profile("category: Language\n")
        self.assertIn("profile.missing-title", found)

    def test_each_unreadable_profile_field_reports_its_own_code(self):
        for fields, code in (
            ("title: ''", "profile.invalid-title"),
            ("title: Python\ncategory: ''", "profile.invalid-category"),
            ("title: Python\ndescription: ''", "profile.invalid-description"),
        ):
            with self.subTest(code=code):
                _, found = self.profile(fields)
                self.assertIn(code, found)

    def test_a_profile_field_the_schema_does_not_define_is_reported(self):
        _, found = self.profile("title: Python\napplies: {terms: [py]}\n")
        self.assertIn("profile.unknown-field", found)

    def test_a_profile_with_no_body_is_refused(self):
        _, found = self.profile("title: Python\n", body="")
        self.assertIn("profile.empty", found)
